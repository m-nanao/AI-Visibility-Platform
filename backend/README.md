...
- レポート出力機能の設計は[docs/26_report_output_design.md](../docs/26_report_output_design.md)を参照。frontendの印刷向けHTMLレポートページ（`/history/[id]/report`）は`feature/report-html-page`（2026-09-11）で最小実装済みだが、**backend側は既存の履歴詳細APIと比較APIをそのまま使い、新規API追加は行っていない**（本番確認・PDF自動生成・共有URL発行はまだ未実装）
