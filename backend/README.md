...
- `services/ai_observation_prompts.py` — Claude/Gemini観測provider（下記）が共通で使う、比較可能性のための統一system/userプロンプトとサマリ化ロジック。既存の`chatgpt_client.py`のプロンプトとは意図的に独立（既存ChatGPT観測の挙動を変えないため）。`SYSTEM_PROMPT`/`build_user_prompt()`/`summarize_observation_text()`
- `services/claude_settings.py` / `claude_client.py` / `claude_provider.py` — Claude相当モデルの1問観測（Anthropic Messages API）。ChatGPT観測と同じ3分割設計。詳細は下記「Claude/Gemini相当モデルの1問観測」参照
- `services/gemini_settings.py` / `gemini_client.py` / `gemini_provider.py` — Gemini相当モデルの1問観測（Google Gemini generateContent API）。同上
...
- `tests/test_main.py`, `tests/test_cooccurrence.py`, `tests/test_cooccurrence_simple.py`, `tests/test_web_fetcher.py`, `tests/test_document_cleaner.py`, `tests/test_document_normalizer.py`, `tests/test_document_chunker.py`, `tests/test_context_analysis.py`, `tests/test_brand_summary.py`, `tests/test_improvement_suggestions.py`, `tests/test_ai_overview_provider.py`, `tests/test_dataforseo_settings.py`, `tests/test_dataforseo_client.py`, `tests/test_chatgpt_settings.py`, `tests/test_chatgpt_client.py`, `tests/test_chatgpt_provider.py`, `tests/test_claude_settings.py`, `tests/test_claude_client.py`, `tests/test_claude_provider.py`, `tests/test_gemini_settings.py`, `tests/test_gemini_client.py`, `tests/test_gemini_provider.py`, `tests/test_common_crawl_settings.py`, `tests/test_common_crawl_index.py`, `tests/test_common_crawl_warc.py`, `tests/test_common_crawl_document_provider.py`, `tests/test_sample_documents.py` — pytestによる最低限のテスト（DataForSEO・OpenAI・Common Crawl関連テストはすべて`httpx`をmonkeypatchで差し替え、実APIへは一切接続しない）
...
### Claude/Gemini相当モデルの1問観測（`services/claude_settings.py` / `claude_client.py` / `claude_provider.py`、`services/gemini_settings.py` / `gemini_client.py` / `gemini_provider.py`、2026-09-15新設）

ChatGPT観測（上記）と同じ設計思想で、`aiOverviewComparison`にさらに2枚、Anthropic Messages APIによる「Claude (Anthropic API)」カードとGoogle Gemini generateContent APIによる「Gemini (Google API)」カードを追加できる**実装基盤**。新設当初（2026-09-15）は基盤の追加のみで本番環境変数は未設定だったが、**2026-09-17時点でClaude側、2026-09-19時点でGemini側もそれぞれRender backend・Vercel frontendの双方に本番環境変数が設定され、本番検証済みである**（下記「Claude providerの現在状態」「Gemini providerの現在状態」参照）。

**注意（重要）**: ChatGPT観測と同様、これらは**Claude/Geminiサービスそのものの内部認識・全体挙動を再現するものではない**。それぞれのAPIモデルへの単発の質問・回答であり、Web検索は使わない。加えて、Claude/Geminiは**同一のプロンプト**（`services/ai_observation_prompts.py`）で質問される——同じブランド・同じ観点で両者の回答を並べて比較できるようにするため（[docs/36_multi_ai_comparison_design.md](../docs/36_multi_ai_comparison_design.md)参照）。既存のChatGPT観測のプロンプトは変更していない（別モジュールで独立して保持）。

**2段階の安全ゲート**（ChatGPT観測と同じ設計）:

1. `CLAUDE_PROVIDER_MODE`/`GEMINI_PROVIDER_MODE`環境変数（未設定時のデフォルトは`off`。無効な値は`off`にフォールバック）。
2. `ALLOW_CLAUDE_MODE_OVERRIDE`/`ALLOW_GEMINI_MODE_OVERRIDE`環境変数（未設定/`false`時はリクエスト単位のoverrideを一切受け付けない）。`true`のときのみ`claudeMode`/`geminiMode`リクエストフィールドが採用される。

この2段階に加え、`mode == "anthropic"`/`"google"`でも、APIキー設定済み・`*_REQUEST_LIMIT_PER_ANALYZE`が`1`の**両方**が揃わない限り外部APIへは一切接続しない。1つでも欠けていれば`meta.claudeProvider.reason`/`meta.geminiProvider.reason`に安全な理由（APIキーの値そのものは含まれない）が入る。

**ChatGPTとの重要な違い（`aiOverviewMode == "mock"`でもスキップしない）**: ChatGPT観測は`aiOverviewMode`が`"mock"`のとき常にスキップされる（mockの固定`aiOverviewComparison`フィクスチャに既に「ChatGPT」という名前のダミーカードがあり、重複を避けるため）。Claude/Geminiにはそのような衝突するmockカードが存在しないため、**`aiOverviewMode`の値に関わらず**、それぞれ独立に`claudeMode`/`geminiMode`のゲートだけで判定される。

**リクエスト仕様**: Anthropic Messages API（`POST https://api.anthropic.com/v1/messages`、`x-api-key`/`anthropic-version`/`content-type: application/json`ヘッダー、`system`はトップレベルフィールド——OpenAIの`role: "system"`メッセージとは異なる形式）。Gemini generateContent API（`POST https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent`、APIキーは`x-goog-api-key`ヘッダーで渡す——URLの`?key=`クエリパラメータは使わない。ログ・プロキシ経由での漏洩リスクを避けるため）。いずれも`httpx`による直接のREST呼び出しで、公式SDK（`anthropic`/`google-genai`）は使わない（`requirements.txt`にまだ含まれておらず、新規ライブラリ追加を避けた）。

**レスポンス変換**: Claudeは`content[]`ブロックのうち`type == "text"`のものを連結。Geminiは`candidates[].content.parts[].text`をたどって連結（安全フィルタでブロックされ`content`自体が存在しない候補も型チェックで安全に無視する）。いずれも読み取れるテキストがなければ`"unavailable"`にフォールバックする。`mentioned`/`summary`（最大200文字）/`fullSummary`（最大2500文字）のロジックはChatGPT観測と同一（`services/ai_observation_prompts.py`の`summarize_observation_text()`）。`rank`は`null`固定、`references`/`referenceSummary`/`ownDomainReferenced`はいずれも`None`固定（参照元の概念がないため）。

**1 analyzeあたりの呼び出し回数**: Claude・Geminiそれぞれ常に最大1回。ChatGPT・DataForSEO・Common Crawlの呼び出し回数・条件には一切影響しない。

**環境変数一覧**: `CLAUDE_API_KEY`（APIキー、空欄可）、`CLAUDE_PROVIDER_MODE`（`off`（デフォルト）/`anthropic`）、`ALLOW_CLAUDE_MODE_OVERRIDE`（`false`（デフォルト）/`true`）、`CLAUDE_MODEL`（デフォルト`claude-sonnet-4-5`）、`CLAUDE_MAX_OUTPUT_TOKENS`（デフォルト`700`、範囲100〜1500）、`CLAUDE_REQUEST_LIMIT_PER_ANALYZE`（デフォルト`1`）。`GEMINI_API_KEY`/`GEMINI_PROVIDER_MODE`（`off`/`google`）/`ALLOW_GEMINI_MODE_OVERRIDE`/`GEMINI_MODEL`（デフォルト`gemini-2.5-flash`）/`GEMINI_MAX_OUTPUT_TOKENS`/`GEMINI_REQUEST_LIMIT_PER_ANALYZE`も同じ形。いずれもフォールバック規則はChatGPT観測の対応する環境変数と同じ。

**開発・検証用UI**: 新設当初（2026-09-15）はフロントエンドの検証用selector UIを追加していなかったが、2026-09-17の`feature/claude-mode-selector`で`NEXT_PUBLIC_ENABLE_CHATGPT_MODE_SELECTOR`相当の`NEXT_PUBLIC_ENABLE_CLAUDE_MODE_SELECTOR`をClaudeのみ追加した（`app/components/BrandInputForm.tsx`、選択肢は`off`/`anthropic`）。Geminiの`geminiMode`は引き続きリクエストボディでのみ指定可能で、selector UIは未追加。既存の`AIOverviewComparisonSection`にはClaude/Gemini観測結果を表示するための受け皿（off/unavailable時の説明表示を含む）を追加済み（詳細は`app/lib/meta-label.ts`/`app/components/sections/AIOverviewComparisonSection.tsx`参照）。

**Claude providerの現在状態（2026-09-17時点）**: Claude providerは実装済みで、frontend selectorによる上書きを通じて本番環境で検証済みである（Claude provider is implemented and production-verified via frontend selector override）。`CLAUDE_PROVIDER_MODE`は引き続きデフォルトで`off`のままであり、`ALLOW_CLAUDE_MODE_OVERRIDE`を明示的な検証のために使う運用としている（`CLAUDE_PROVIDER_MODE` remains off by default, with `ALLOW_CLAUDE_MODE_OVERRIDE` used for explicit verification）。Claude API keyはRender backendの環境変数にのみ保存されている（Claude API key is stored only in Render backend environment variables）。本番Vercel環境変数`NEXT_PUBLIC_ENABLE_CLAUDE_MODE_SELECTOR=true`と組み合わせ、本番画面でClaude API選択→分析実行→結果カード表示→履歴詳細→レポート表示までの一連の動作を確認済み（詳細は[docs/36_multi_ai_comparison_design.md](../docs/36_multi_ai_comparison_design.md)「10. Claude観測の本番検証結果」参照）。

**Gemini providerの現在状態（2026-09-19時点）**: Gemini providerは実装済みで、frontend selectorによる上書きを通じて本番環境で検証済みである（Gemini provider is implemented and production-verified via frontend selector override）。`GEMINI_PROVIDER_MODE`は引き続きデフォルトで`off`のままであり、`ALLOW_GEMINI_MODE_OVERRIDE`を明示的な検証のために使う運用としている（`GEMINI_PROVIDER_MODE` remains off by default, with `ALLOW_GEMINI_MODE_OVERRIDE` used for explicit verification）。Gemini API keyはRender backendの環境変数にのみ保存されている（Gemini API key is stored only in Render backend environment variables）。本番Vercel環境変数`NEXT_PUBLIC_ENABLE_GEMINI_MODE_SELECTOR=true`と組み合わせ、本番画面でGemini API選択→分析実行→結果カード表示→履歴詳細→レポート表示までの一連の動作を確認済み。**本番`GEMINI_MAX_OUTPUT_TOKENS`は700（デフォルト）で本文が途中で切れる現象が確認されたため、1500へ調整し解消を確認済みである**（`GEMINI_MAX_OUTPUT_TOKENS=1500 was verified to avoid the short truncation observed with 700 tokens`）。途中終了検知・注意文表示の実装（`services/gemini_client.py`の`_extract_finish_reason()`/`_is_likely_truncated()`、`AIOverviewComparisonItem`の`isTruncated`/`note`フィールド）も導入済みである（truncation warning support is implemented）。詳細は[docs/36_multi_ai_comparison_design.md](../docs/36_multi_ai_comparison_design.md)「12. Gemini観測結果の途中終了検知と表示改善」「13. Gemini観測の本番検証結果」参照。
...
- **有効化条件**: 環境変数`READ_HISTORY_ENABLED=true`かつ`DATABASE_URL`が設定されている場合のみ、DB接続を試みる（`services/db_settings.py`の`is_history_read_enabled()`）。**`DB_SAVE_ENABLED=true`だけではread APIは有効にならない**——保存はbackend内部処理だが、read APIは外部からアクセスされるため安全性の扱いが異なり、独立したフラグにしている（[docs/20_analysis_history_read_api_design.md](../docs/20_analysis_history_read_api_design.md)「9. 認証未実装期間の公開範囲」参照）。**2026-09-10追加: 上記2条件に加えて`HISTORY_READ_TOKEN`環境変数の設定も必須**（`services/db_settings.py`の`is_history_read_token_configured()`）。リクエストの`X-History-Read-Token`ヘッダーがこの値と一致しない限り両エンドポイントとも403を返す（`main.py`の`_check_history_read_access()`、`hmac.compare_digest()`で比較。token値はレスポンス・ログのいずれにも出さない）。詳細は[docs/24_auth_rls_history_access_design.md](../docs/24_auth_rls_history_access_design.md)「7. backend APIでのアクセス制御案」「14. 実装状況」参照。**本番環境で、backend直アクセス（Render backend URLへの直接アクセス）はHTTP 403で拒否され、Vercelのserver-side proxy経由では履歴一覧が表示されることを確認済み**（2026-09-10、[docs/24_auth_rls_history_access_design.md](../docs/24_auth_rls_history_access_design.md)「15. 本番環境での動作確認」参照）。
...
- **`GET /analysis-runs/{id}/comparison`**（履歴比較、2026-09-10新設、`feature/history-comparison-api`）: 指定idの履歴（`current`）と、同一ブランド（`brand_id`のみで判定、`canonical_domain`補助は未実装）の直近前回履歴（`previous`）を比較する（設計は[docs/25_analysis_history_comparison_design.md](../docs/25_analysis_history_comparison_design.md)参照）。`services/analysis_history_repository.py`の`get_previous_analysis_run_for_brand()`が`analysis_runs`の自己結合1クエリで前回履歴を取得し、`services/analysis_history_comparison.py`の純粋関数`build_comparison_response()`が`visibilityScore`差分・共起語ランキング上位10件の`newTerms`/`removedTerms`/`changedTerms`（`rankDelta`/`scoreDelta`はcurrent-previous、順位改善時は`rankDelta`が負）・改善提案の件数差分を算出する。`GET /analysis-runs/{analysis_run_id}`より前にroute定義しているが、Starletteのルーティングはパスのセグメント数で区別するため実害はない。**上記2エンドポイントと同じ`HISTORY_READ_TOKEN`gate（`_check_history_read_access()`）が適用される**。currentが存在しない場合は404、previousが存在しない場合は200で`previous: null`・`diff: null`・`warnings: ["比較できる過去履歴がまだありません。"]`。AI Overview provider modeが一致しない場合や`result`が旧schema等で欠損している場合は`warnings`に注意文言を追加する。**frontend proxy route（`app/api/analysis-runs/[id]/comparison/route.ts`）・`/history/[id]`への「前回比較」セクションは`feature/history-comparison-ui`（2026-09-10）で実装済み**（frontend側の詳細は`../README.md`または[docs/25_analysis_history_comparison_design.md](../docs/25_analysis_history_comparison_design.md)「20. 実装状況」参照）。**履歴比較UIは本番環境で確認済み**——`/history/[id]`の「前回比較」セクションで、前回履歴がある場合はスコア差分・共起語変化・改善提案数差分を表示し、前回履歴がない場合はデータ不足メッセージを表示することを確認した（2026-09-10、[docs/25_analysis_history_comparison_design.md](../docs/25_analysis_history_comparison_design.md)「21. 本番環境での動作確認」参照）。**DB schema変更・migration変更はいずれも行っていない**。
- **エラー時の挙動**: `READ_HISTORY_ENABLED=false`は503（`"analysis history read API is not enabled"`）、`DATABASE_URL`未設定は503（`"analysis history read API is not configured"`）、`HISTORY_READ_TOKEN`未設定は503（`"analysis history read token is not configured"`、2026-09-10追加）——いずれも原因を区別できるよう異なるメッセージにしている。`X-History-Read-Token`ヘッダーなし・不一致は403（`"analysis history read access denied"`、2026-09-10追加）。DB接続/クエリ失敗は503（`"failed to read analysis history"`）。対象IDなしは404。不正な`limit`/`offset`はFastAPIの標準validationにより400（既存の`/analyze`と同じ`{"error": "invalid request body"}`形式、`main.py`の`validation_exception_handler`を共有）。**保存処理（`save_analysis_history()`）と異なり、read APIは失敗を握りつぶさない**——`list_analysis_runs()`/`get_analysis_run()`は接続・クエリ失敗時に`AnalysisHistoryReadError`を送出し、`main.py`側で503に変換する。
...
- **frontend履歴一覧UI**: [docs/21_analysis_history_ui_design.md](../docs/21_analysis_history_ui_design.md)の設計に沿って、`app/history/page.tsx`＋`app/api/analysis-runs/route.ts`（このbackendの`GET /analysis-runs`への薄いプロキシ）として最小実装済み（2026-09-10、`feature/history-list-ui`）。一覧のみで、履歴詳細UI（`/history/[id]`）はまだ実装していない。frontend側の詳細は`../README.md`または上記docsを参照——このファイル（backend README）は引き続きbackend APIの仕様のみを扱う。**`/history`は`READ_HISTORY_ENABLED=false`時の無効表示と、`READ_HISTORY_ENABLED=true`時の保存済み履歴表示の両方を本番環境で確認済み**（2026-09-10、[docs/21_analysis_history_ui_design.md](../docs/21_analysis_history_ui_design.md)「15. 本番Vercel環境での動作確認」参照）。**認証未実装のため、`READ_HISTORY_ENABLED=true`のままにすると履歴データが誰でも表示可能になる——通常運用では`false`に戻すことを推奨する。**
- **frontend履歴詳細UI**: `GET /analysis-runs/{id}`を使った履歴詳細画面（`/history/[id]`）を[docs/22_analysis_history_detail_ui_design.md](../docs/22_analysis_history_detail_ui_design.md)の設計に沿って最小実装済み（2026-09-10、`feature/history-detail-ui`）。`app/api/analysis-runs/[id]/route.ts`（このbackendの`GET /analysis-runs/{id}`への薄いプロキシ、503/404はそのまま転送）と、既存の`app/components/AnalysisDashboard.tsx`をそのまま再利用した保存済み`result`表示、`result`が現在の`AnalysisResult`形式と一致しない場合の互換性エラー表示を含む。frontend側の詳細は`../README.md`または上記docsを参照。**`/history/[id]`履歴詳細UIは、`READ_HISTORY_ENABLED=false`時の無効表示と、`READ_HISTORY_ENABLED=true`時の保存済み詳細表示を本番環境で確認済み**（2026-09-10、[docs/22_analysis_history_detail_ui_design.md](../docs/22_analysis_history_detail_ui_design.md)「17. 本番Vercel環境での動作確認」参照）。**認証未実装のため、`READ_HISTORY_ENABLED=true`のままにすると履歴一覧・履歴詳細が誰でも表示可能になる。通常は`false`運用を推奨する。** `analysisRunId`追加と分析直後リンクの設計は[docs/23_analysis_run_id_and_post_analyze_link_design.md](../docs/23_analysis_run_id_and_post_analyze_link_design.md)を参照。**`/analyze`レスポンスへの`analysisRunId`追加は`feature/analysis-run-id-response`（2026-09-10、詳細は上記「レスポンス形状について」参照）、分析結果画面への「保存済み履歴で開く」リンク表示は`feature/post-analyze-history-link`（2026-09-10）でそれぞれ実装済み**——`analysisRunId`が存在する場合のみ既存の`AnalysisDashboard.tsx`にリンクを表示し、`null`/`undefined`の場合は非表示のまま。frontend側はこのbackendの`READ_HISTORY_ENABLED`の状態を判定せず、無効時の表示はリンク先の`/history/[id]`に任せる。**`/analyze`実行後、DB保存成功時に`analysisRunId`が返り、frontendの分析結果画面に「保存済み履歴で開く」リンクが表示されることを本番環境で確認済み**（2026-09-10、[docs/23_analysis_run_id_and_post_analyze_link_design.md](../docs/23_analysis_run_id_and_post_analyze_link_design.md)「16. 本番Vercel環境での動作確認」参照）。**認証/RLS・履歴アクセス制御の設計は[docs/24_auth_rls_history_access_design.md](../docs/24_auth_rls_history_access_design.md)を参照**——うち`HISTORY_READ_TOKEN`によるbackend read API token gateは`feature/history-read-token-gate`（2026-09-10）で実装済み（詳細は上記「有効化条件」「エラー時の挙動」参照）。Supabase Auth/RLSはまだ未実装。
...
| `meta.claudeProvider` | （任意、2026-09-15追加）Claude相当モデルの1問観測（Anthropic API）が`aiOverviewComparison`にカードを追加したかどうか（`{mode, status, reason, environment}`）。`mode`は`"off"`/`"anthropic"`、`status`は`"real"`/`"off"`/`"unavailable"`、`environment`は`"api"`/`"off"`/`"unavailable"`。他のprovider（`aiOverviewProvider`/`chatgptProvider`/`geminiProvider`）とは完全に独立で、ChatGPTと異なり`aiOverviewMode=="mock"`でもスキップされない。`reason`はAnthropic設定状態を安全に説明するがAPIキーの値は含まない。詳細は上記「Claude/Gemini相当モデルの1問観測」参照 |
| `meta.geminiProvider` | （任意、2026-09-15追加）Gemini相当モデルの1問観測（Google Gemini API）が`aiOverviewComparison`にカードを追加したかどうか（`{mode, status, reason, environment}`）。`mode`は`"off"`/`"google"`、`status`/`environment`/独立性・mock非スキップの扱いは`claudeProvider`と同じ。詳細は上記「Claude/Gemini相当モデルの1問観測」参照 |
...
| `analysisRunId` | （任意、2026-09-10追加）DB保存（`DB_SAVE_ENABLED=true`かつ`DATABASE_URL`設定済み）に成功した場合のみ、保存された`analysis_runs.id`（UUID文字列）が入る。保存無効・`DATABASE_URL`未設定・保存失敗のいずれも`null`。DB保存の成否は`/analyze`自体の成否と独立しており、保存が`null`でも`/analyze`は通常どおり成功する。分析結果画面へのリンク表示（`analysisRunId`が存在する場合のみ）は`feature/post-analyze-history-link`（2026-09-10）で実装済み。詳細は[docs/23_analysis_run_id_and_post_analyze_link_design.md](../docs/23_analysis_run_id_and_post_analyze_link_design.md)参照 |
...
- PostgreSQL/Supabaseの本格活用（**Supabase Free環境への実DB接続・最小保存（`brands`/`analysis_runs`/`analysis_results`）、保存済み履歴を読む最小read API（`GET /analysis-runs`/`GET /analysis-runs/{id}`、`READ_HISTORY_ENABLED`デフォルトoff）、frontend履歴一覧UI（`/history`）・履歴詳細UI（`/history/[id]`）の最小実装、および両UIの本番Vercel環境での動作確認まで完了済み**（2026-09-10）。詳細は上記「分析履歴のDB保存」「保存済み分析履歴を読むread API」「frontend履歴一覧UI」「frontend履歴詳細UI」参照。**`analysisRunId`のAPIレスポンス追加・分析結果画面への「保存済み履歴で開く」リンク表示も完了済み**（2026-09-10、`feature/analysis-run-id-response`/`feature/post-analyze-history-link`、詳細は[docs/23_analysis_run_id_and_post_analyze_link_design.md](../docs/23_analysis_run_id_and_post_analyze_link_design.md)参照）。**認証/RLS・履歴アクセス制御の設計、および`HISTORY_READ_TOKEN`によるbackend read API token gateの最小実装も完了済み**（2026-09-10、`feature/history-read-token-gate`、[docs/24_auth_rls_history_access_design.md](../docs/24_auth_rls_history_access_design.md)参照）。**履歴比較機能の設計、`GET /analysis-runs/{id}/comparison`の最小実装、`/history/[id]`への前回比較セクション・frontend proxy routeの最小実装、および履歴比較UIの本番確認も完了済み**（2026-09-10、`feature/history-comparison-api`/`feature/history-comparison-ui`、[docs/25_analysis_history_comparison_design.md](../docs/25_analysis_history_comparison_design.md)参照——詳細は上記「`GET /analysis-runs/{id}/comparison`」参照）。pgvector導入・非同期job化・観測系の個別テーブル化・Supabase Auth/RLS・比較専用ページ・手動比較対象選択・レポート出力はいずれもまだ行っていない）
- ChatGPT観測（`chatgpt_provider.py`）の常時運用（現状はデフォルト`off`・1 analyzeあたり最大1回の手動/検証用途のみ。複数質問・DB保存・課金管理を伴う本番運用は対象外）
- **Claude/Gemini観測の実装基盤は追加済み**（2026-09-15、`claude_provider.py`/`gemini_provider.py`、詳細は上記「Claude/Gemini相当モデルの1問観測」参照）。**Claude側は2026-09-17時点、Gemini側は2026-09-19時点で、それぞれ開発・検証用UI selector追加・本番Render/Vercel環境変数設定・本番検証まで完了済み**（上記「Claude providerの現在状態」「Gemini providerの現在状態」参照）。ChatGPT観測へのWeb検索（`web_search`ツール）・参照元付き回答の追加、Claude/Gemini/ChatGPT横断の高度な時系列比較UIも対象外
...
- Supabase Auth/RLS migration設計は[docs/28_supabase_auth_rls_migration_design.md](../docs/28_supabase_auth_rls_migration_design.md)を参照。既存データを壊さず、default organization/default projectを経由して段階的に移行する方針。migration SQL案`backend/migrations/002_add_organizations_projects.sql`を追加済み（`organizations`/`projects`/`organization_members`テーブル作成、`brands`/`analysis_runs`へのNULL許容`project_id`追加、default organization/projectの冪等作成とbackfillを含む。RLS有効化・NOT NULL制約・既存データ削除は含まない）。検証用Supabase projectでの実行確認手順は[docs/29_supabase_migration_verification_guide.md](../docs/29_supabase_migration_verification_guide.md)を参照。**`002_add_organizations_projects.sql`は本番Supabaseへ適用済み**（本番適用手順・適用結果は[docs/30_supabase_production_migration_002_runbook.md](../docs/30_supabase_production_migration_002_runbook.md)を参照）。`organizations`/`projects`/`organization_members`、`brands.project_id`、`analysis_runs.project_id`は本番DBに追加済みで、既存データはdefault projectへbackfill済み。**新規保存時にdefault project_idを付与するbackend対応は実装済み**（`save_analysis_history()`が`get_default_project_id()`で取得したdefault project_idを新規`brands`/`analysis_runs`へ設定し、既存brandの`project_id`が既に設定されている場合は上書きしない。default projectが見つからない場合は保存自体をスキップし、`/analyze`本体は失敗させない）。**002適用直後に発生した`analysis_runs.project_id`が`null`の1件は、本番Supabase SQL Editorでの手動backfillにより解消済み**（SQL案は[docs/30_supabase_production_migration_002_runbook.md](../docs/30_supabase_production_migration_002_runbook.md)「19. 新規保存時のproject_id付与と手動backfill」を参照）——backfill後、`analysis_runs.project_id`/`brands.project_id`が`null`の件数はいずれも0を確認済み。本番のRLS確認では対象6テーブルの`relrowsecurity`が`true`だったが`pg_policies`は0行——002 migration自体にはRLS有効化SQLを含まず、本番でのRLS操作・policy追加は行っていない。

- **現在の認証状態（2026-09-12）**: frontendではSupabase Auth Email + Passwordログイン（`/login`、`app/components/AuthGuard.tsx`）が導入済みで、`/history`・`/history/[id]`・`/history/[id]/report`はSupabase Auth session必須になっており、frontend proxy route（`app/api/analysis-runs/*`）が取得できた場合のみSupabase access tokenを`Authorization: Bearer <token>`としてこのbackendへ送る（`@supabase/ssr`によるcookieベースsession化を含め、mainへ反映済み・本番Vercelでの動作確認も完了済み。詳細は[docs/34_supabase_auth_introduction_design.md](../docs/34_supabase_auth_introduction_design.md)「15」「16」「19」「20」を参照）。**このbackend側では、`feature/backend-history-api-jwt-project-access`（2026-09-12）で、JWT検証（`services/jwt_auth.py`）とproject権限判定（`services/project_access.py`）を`GET /analysis-runs`・`GET /analysis-runs/{analysis_run_id}`・`GET /analysis-runs/{analysis_run_id}/comparison`の3本すべてに実際に接続した**——新規`HistoryAccessContext`/`_resolve_history_access()`（`main.py`）が、`X-History-Read-Token`が正しい場合は従来どおり無制限に許可し（**既存frontend proxyの本番挙動は完全に不変**）、それ以外で`AUTH_JWT_ENABLED=true`かつ有効なJWTがある場合のみ`user_id`が所属するprojectのデータに絞り込む。**JWTだけで全履歴が返ることはない**——project権限なしは403、判定処理自体のエラーは503（fail-closed）。comparisonは現在のrunの`project_id`で前回runの範囲を限定し、project境界を越えて返らないようにした。詳細・エラー設計は[docs/32_backend_jwt_verification_design.md](../docs/32_backend_jwt_verification_design.md)「18. backend履歴APIへのJWT検証＋project権限判定の接続」を参照。**Render本番環境では`AUTH_JWT_ENABLED=true`を設定済み**（`AUTH_PROVIDER=supabase`・`SUPABASE_JWKS_URL`・`SUPABASE_JWT_ISSUER`・`SUPABASE_JWT_AUDIENCE`も設定、`SUPABASE_SERVICE_ROLE_KEY`・`SUPABASE_JWT_SECRET`は未設定）——Render再デプロイ後、本番Vercelで`/login`・`/history`系3ページの表示・ログアウト・ログアウト後の`/login`誘導を確認し、既存の履歴表示が壊れていないことを確認済み。**この段階では`HISTORY_READ_TOKEN`互換経路がJWTより優先されており、両方のheaderを送るfrontend proxyの本番リクエストは実質的にHISTORY_READ_TOKEN経路を通っていた**（当時の状態の記録として残す）。**これは`feature/prefer-jwt-project-access-for-history-api`（2026-09-12）で入れ替えられている**——`AUTH_JWT_ENABLED=true`かつ`Authorization`headerがある場合はJWT/project権限判定経路が最優先になり、`HISTORY_READ_TOKEN`が正しくても参照しない（fallbackしない：JWTが不正・期限切れ・検証設定不足の場合もHISTORY_READ_TOKENへは戻らず401/503を返す）。`Authorization`headerが無い場合のみ、従来どおり`HISTORY_READ_TOKEN`が使われる。backendのみの変更で、frontend/Supabase/Render/Vercel設定・migration・RLSはいずれも変更していない。詳細は[docs/32_backend_jwt_verification_design.md](../docs/32_backend_jwt_verification_design.md)「19. AUTH_JWT_ENABLED=true の本番有効化と互換確認」「20. JWTがある場合にJWT/project権限判定を優先する移行実装」を参照。**JWT/project権限判定経路が実際に優先利用されることは、`docs/record-jwt-project-access-production-verification`（2026-09-12）で本番確認済みである**——frontend proxyから送られる`Authorization: Bearer`をbackendが優先し、`/login`・`/history`・`/history/[id]`・`/history/[id]/report`・ログアウトが、JWT検証＋project権限判定を通った状態で正常に動作することを本番Vercelで確認した。`HISTORY_READ_TOKEN`は`Authorization`headerがない場合の互換fallbackとして引き続き維持されている。**RLS policyはまだ本番適用していない**——backendのproject権限判定が動作していることは、RLS policyというDB層の追加防御が不要であることを意味しない。詳細は[docs/32_backend_jwt_verification_design.md](../docs/32_backend_jwt_verification_design.md)「21. JWT/project権限判定経路の本番確認」を参照。**RLS policyを検証DBで確認するための実行手順は[docs/35_rls_verification_runbook.md](../docs/35_rls_verification_runbook.md)として整備済み**（検証データ作成手順・`drop policy if exists`付きSQL・検証方法・期待結果・rollback SQL・本番適用前チェックリストを含む）。**この手順に沿って02_migration検証用Supabase projectでRLS select policyを実際に適用し、基本テストを実施済みである**——user A2（organization Aのみ所属）はorganization/project/brandいずれもA側のみ、user B（organization Bのみ所属）はB側のみをREST API + JWT経由で確認でき、相手側データへのアクセス・漏えいは確認されなかった。未認証アクセスは`permission denied`（検証DBでは`anon`roleにSELECT権限をgrantしていないため）で、未認証で読めていないことが確認できていれば安全側としてOKと判断する。`organization_members`の再帰エラーも発生しなかった。**本番DBには今回もRLS policyを適用していない。** `analysis_runs`/`analysis_results`のREST可視性確認は同じ検証DB・同じuser A2/Bで追加確認済みである——**RLS select policyは02_migration検証DBで、organizations / projects / brands / analysis_runs / analysis_results の分離確認済み。analysis_resultsは親analysis_runs経由でproject境界内に制限されることを確認済み**。未認証アクセスはanalysis_runs/analysis_resultsいずれも読めないことも確認した。検証途中でaccess tokenが期限切れとなり`JWT expired`が返る場面があったが、テストユーザーで再ログインしてaccess tokenを再取得すれば検証を継続できる。**本番DBには今回もRLS policyを適用していない。** rollback SQLの検証DB実行も完了済みである——**RLS select policyは02_migration検証DBで、適用・REST/JWT分離確認（organizations / projects / brands / analysis_runs / analysis_results）・rollback SQL確認まで完了している**。rollback実行によりselect policy 6件が削除され、対象6テーブルの`rls_enabled`/`rls_forced`がいずれも`false`に戻り、helper function `public.is_org_member(uuid)`も削除されたことを確認した（テーブル本体・検証データは削除していない）。**本番DBには今回もRLS policyを適用していない。** 本番適用判断は引き続き未確認のまま残る。詳細は[docs/35_rls_verification_runbook.md](../docs/35_rls_verification_runbook.md)「12. 検証DBでの基本テスト結果」「13. analysis_runs / analysis_resultsのRLS可視性確認結果」「14. rollback SQLの検証DB実行結果」を参照。
