...
- **現在の認証状態（2026-09-11）**: frontendではSupabase Auth Email + Passwordログイン（`/login`、`app/components/AuthGuard.tsx`）が導入済みで、`/history`・`/history/[id]`・`/history/[id]/report`はSupabase Auth session必須になっている（`feature/supabase-auth-frontend-login`、本番Vercelでの動作確認・env設定・Supabaseユーザー作成も完了済み。詳細は[docs/34_supabase_auth_introduction_design.md](../docs/34_supabase_auth_introduction_design.md)「15. 実装状況」「16. 本番環境での動作確認」を参照）。**ただしこのbackend自体はまだSupabase JWT検証を行っておらず**、read APIへのアクセス制御は引き続き`HISTORY_READ_TOKEN` gate（上記「有効化条件」参照）が担っている——frontend proxy routeがSupabase access tokenをこのbackendへ送る処理はまだ実装していない。backend JWT検証の設計は[docs/32_backend_jwt_verification_design.md](../docs/32_backend_jwt_verification_design.md)を参照（**まだ実装ではない**）。
