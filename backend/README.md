...
| `analysisRunId` | （任意、2026-09-10追加）DB保存（`DB_SAVE_ENABLED=true`かつ`DATABASE_URL`設定済み）に成功した場合のみ、保存された`analysis_runs.id`（UUID文字列）が入る。保存無効・`DATABASE_URL`未設定・保存失敗のいずれも`null`。DB保存の成否は`/analyze`自体の成否と独立しており、保存が`null`でも`/analyze`は通常どおり成功する。分析結果画面へのリンク表示はまだ実装していない。詳細は[docs/23_analysis_run_id_and_post_analyze_link_design.md](../docs/23_analysis_run_id_and_post_analyze_link_design.md)参照 |
...
- PostgreSQL/Supabaseの本格活用（**Supabase Free環境への実DB接続・最小保存（`brands`/`analysis_runs`/`analysis_results`）、保存済み履歴を読む最小read API（`GET /analysis-runs`/`GET /analysis-runs/{id}`、`READ_HISTORY_ENABLED`デフォルトoff）、frontend履歴一覧UI（`/history`）・履歴詳細UI（`/history/[id]`）の最小実装、および両UIの本番Vercel環境での動作確認まで完了済み**（2026-09-10）。詳細は上記「分析履歴のDB保存」「保存済み分析履歴を読むread API」「frontend履歴一覧UI」「frontend履歴詳細UI」参照。`analysisRunId`のAPIレスポンス追加・pgvector導入・非同期job化・観測系の個別テーブル化・認証/RLSはいずれもまだ行っていない）
