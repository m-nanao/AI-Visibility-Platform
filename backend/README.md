...
- **現在の認証状態（2026-09-11）**: frontendではSupabase Auth Email + Passwordログイン（`/login`、`app/components/AuthGuard.tsx`）が導入済みで、`/history`・`/history/[id]`・`/history/[id]/report`はSupabase Auth session必須になっている（`feature/supabase-auth-frontend-login`、本番Vercelでの動作確認・env設定・Supabaseユーザー作成も完了済み。詳細は[docs/34_supabase_auth_introduction_design.md](../docs/34_supabase_auth_introduction_design.md)「15. 実装状況」「16. 本番環境での動作確認」を参照）。**このbackend側のJWT検証module（`services/auth_settings.py`・`services/jwt_auth.py`、JWKS方式でSupabase access tokenを検証し`user_id`を取得）は`feature/backend-jwt-verification`（2026-09-11）で追加済みだが、read API（`GET /analysis-runs`系3本）へはまだ組み込んでいない**——アクセス制御は引き続き`HISTORY_READ_TOKEN` gate（上記「有効化条件」参照）のみが担っており、`AUTH_JWT_ENABLED=true`にしてAuthorization headerを送っても許可条件は変わらないことをテストで確認済み。**frontend proxy route（`app/api/analysis-runs/*`）は`feature/frontend-proxy-forward-auth-token`（2026-09-11）で、取得できた場合のみSupabase access tokenを`Authorization: Bearer <token>`としてこのbackendへ送るようになった**が、このbackend側はまだそのheaderを検証条件に使っていない（`X-History-Read-Token`は変更なく維持）。**このaccess token転送はmainへ反映済みで、本番Vercelでの実ブラウザ動作確認（`/login`アクセス・既存ユーザーログイン・`/history`系3ページ表示・ログアウト・ログアウト後の`/login`誘導）も完了済み**——ただし今回もこのbackend側の許可条件（`HISTORY_READ_TOKEN`のみ）は未変更のまま。backend JWT検証の設計・実装状況は[docs/32_backend_jwt_verification_design.md](../docs/32_backend_jwt_verification_design.md)「14. 実装状況」「16. frontend→backend access token転送の実装状況」「17. frontend→backend access token転送の本番確認」を参照。**project権限判定のhelper（`services/project_access.py`の`can_user_access_project()`/`can_user_access_analysis_run()`/`get_accessible_project_ids()`、`feature/backend-project-access-helpers`、2026-09-11）も追加済みだが、こちらも`main.py`からはまだ呼んでおらず**、read APIの許可条件は変わらない。詳細は[docs/32_backend_jwt_verification_design.md](../docs/32_backend_jwt_verification_design.md)「15. project権限判定の実装状況」を参照。**RLS policy本番適用・Render env設定はいずれもまだ行っていない**。
