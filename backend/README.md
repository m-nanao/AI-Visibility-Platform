...
- Supabase Auth/RLS migration設計は[docs/28_supabase_auth_rls_migration_design.md](../docs/28_supabase_auth_rls_migration_design.md)を参照。既存データを壊さず、default organization/default projectを経由して段階的に移行する方針。migration SQL案`backend/migrations/002_add_organizations_projects.sql`を追加済み（`organizations`/`projects`/`organization_members`テーブル作成、`brands`/`analysis_runs`へのNULL許容`project_id`追加、default organization/projectの冪等作成とbackfillを含む。RLS有効化・NOT NULL制約・既存データ削除は含まない。**本番Supabaseへの適用・RLS有効化はまだ行っていない**）。002 migrationの本番適用前検証手順は[docs/29_supabase_migration_verification_guide.md](../docs/29_supabase_migration_verification_guide.md)を参照。`002_add_organizations_projects.sql`は検証用Supabase projectで実行確認済み。テーブル追加、project_id追加、default organization/project作成、既存データbackfill、冪等性、RLS未有効状態を確認済み。本番Supabaseにはまだ適用していない。002 migrationの本番適用手順は[docs/30_supabase_production_migration_002_runbook.md](../docs/30_supabase_production_migration_002_runbook.md)を参照。本番適用時はbackup/件数確認後に実行し、RLSはこのmigrationでは有効化しない。
