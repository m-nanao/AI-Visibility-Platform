...
ChatGPT観測（上記）と同じ設計思想で、`aiOverviewComparison`にさらに2枚、Anthropic Messages APIによる「Claude (Anthropic API)」カードとGoogle Gemini generateContent APIによる「Gemini (Google API)」カードを追加できる**実装基盤**。今回のタスクは基盤の追加のみで、**本番Render/Vercel環境でのAPIキー設定・実際の有効化は対象外**（別タスク）——`CLAUDE_PROVIDER_MODE`/`GEMINI_PROVIDER_MODE`のデフォルトは`off`で、本番環境変数には設定していない。
...
**開発・検証用UI**: 今回のタスクではフロントエンドの検証用selector UI（`NEXT_PUBLIC_ENABLE_CHATGPT_MODE_SELECTOR`相当）は追加していない——`claudeMode`/`geminiMode`はリクエストボディでのみ指定可能。既存の`AIOverviewComparisonSection`にはClaude/Gemini観測結果を表示するための受け皿（off/unavailable時の説明表示を含む）を追加済み（詳細は`app/lib/meta-label.ts`/`app/components/sections/AIOverviewComparisonSection.tsx`参照）。
...
- **Claude/Gemini観測の実装基盤は追加済み**（2026-09-15、`claude_provider.py`/`gemini_provider.py`、詳細は上記「Claude/Gemini相当モデルの1問観測」参照）。ただし本番Render/Vercel環境でのAPIキー設定・実際の有効化、開発・検証用UI selector（`NEXT_PUBLIC_ENABLE_CHATGPT_MODE_SELECTOR`相当）の追加はいずれもまだ行っていない（別タスク）。ChatGPT観測へのWeb検索（`web_search`ツール）・参照元付き回答の追加、Claude/Gemini/ChatGPT横断の高度な時系列比較UIも対象外
