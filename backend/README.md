...
- PostgreSQL/Supabaseの本格活用（**Supabase Free環境への実DB接続・最小保存（`brands`/`analysis_runs`/`analysis_results`）、保存済み履歴を読む最小read API（`GET /analysis-runs`/`GET /analysis-runs/{id}`、`READ_HISTORY_ENABLED`デフォルトoff）、frontend履歴一覧UI（`/history`）・履歴詳細UI（`/history/[id]`）の最小実装まで完了済み**（2026-09-10）。詳細は上記「分析履歴のDB保存」「保存済み分析履歴を読むread API」「frontend履歴一覧UI」「frontend履歴詳細UI」参照。`analysisRunId`のAPIレスポンス追加・pgvector導入・非同期job化・観測系の個別テーブル化・認証/RLSはいずれもまだ行っていない）
