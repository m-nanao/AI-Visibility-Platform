...
ChatGPT観測（上記）と同じ設計思想で、`aiOverviewComparison`にさらに2枚、Anthropic Messages APIによる「Claude (Anthropic API)」カードとGoogle Gemini generateContent APIによる「Gemini (Google API)」カードを追加できる**実装基盤**。新設当初（2026-09-15）は基盤の追加のみで本番環境変数は未設定だったが、**2026-09-17時点でClaude側はRender backend・Vercel frontendの双方に本番環境変数が設定され、本番検証済みである**（下記「Claude providerの現在状態」参照）。Geminiは引き続き`GEMINI_PROVIDER_MODE`のデフォルト`off`のまま、本番環境変数は未設定。
...
- **Claude/Gemini観測の実装基盤は追加済み**（2026-09-15、`claude_provider.py`/`gemini_provider.py`、詳細は上記「Claude/Gemini相当モデルの1問観測」参照）。**Claude側は2026-09-17時点で開発・検証用UI selector追加・本番Render/Vercel環境変数設定・本番検証まで完了済み**（上記「Claude providerの現在状態」参照）。**Gemini側は本番Render/Vercel環境でのAPIキー設定・実際の有効化・開発検証用UI selectorの追加のいずれもまだ行っていない**。ChatGPT観測へのWeb検索（`web_search`ツール）・参照元付き回答の追加、Claude/Gemini/ChatGPT横断の高度な時系列比較UIも対象外
