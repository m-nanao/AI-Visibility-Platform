...
- PostgreSQL/Supabaseの本格活用（**Supabase Free環境への実DB接続・最小保存（`brands`/`analysis_runs`/`analysis_results`）、保存済み履歴を読む最小read API（`GET /analysis-runs`/`GET /analysis-runs/{id}`、`READ_HISTORY_ENABLED`デフォルトoff）、frontend履歴一覧UI（`/history`）・履歴詳細UI（`/history/[id]`）の最小実装、および両UIの本番Vercel環境での動作確認まで完了済み**（2026-09-10）。詳細は上記「分析履歴のDB保存」「保存済み分析履歴を読むread API」「frontend履歴一覧UI」「frontend履歴詳細UI」参照。**`analysisRunId`のAPIレスポンス追加・分析結果画面への「保存済み履歴で開く」リンク表示も完了済み**（2026-09-10、`feature/analysis-run-id-response`/`feature/post-analyze-history-link`、詳細は[docs/23_analysis_run_id_and_post_analyze_link_design.md](../docs/23_analysis_run_id_and_post_analyze_link_design.md)参照）。**認証/RLS・履歴アクセス制御の設計メモも追加済み**（2026-09-10、[docs/24_auth_rls_history_access_design.md](../docs/24_auth_rls_history_access_design.md)参照——短期は`READ_HISTORY_ENABLED=false`継続、中期は`HISTORY_READ_TOKEN`によるbackend read API token gateを推奨）。pgvector導入・非同期job化・観測系の個別テーブル化・認証/RLSの実装自体はいずれもまだ行っていない）
