...
- **frontend履歴詳細UI設計**: `GET /analysis-runs/{id}`を使った履歴詳細画面（`/history/[id]`）の設計メモを[docs/22_analysis_history_detail_ui_design.md](../docs/22_analysis_history_detail_ui_design.md)として整理済み（2026-09-10）。既存分析結果コンポーネントの再利用方針・保存済み`result_json`をスナップショットとして扱う方針・503/404/schema validation失敗時の表示方針を整理しているが、実装はまだ行っていない。
...
- PostgreSQL/Supabaseの本格活用（**Supabase Free環境への実DB接続・最小保存（`brands`/`analysis_runs`/`analysis_results`）、保存済み履歴を読む最小read API（`GET /analysis-runs`/`GET /analysis-runs/{id}`、`READ_HISTORY_ENABLED`デフォルトoff）、およびfrontend履歴一覧UI（`/history`）の最小実装・本番Vercel環境での動作確認まで完了済み**（2026-09-10）。詳細は上記「分析履歴のDB保存」「保存済み分析履歴を読むread API」「frontend履歴一覧UI」参照。分析履歴の詳細閲覧UI（`/history/[id]`）は設計メモ（[docs/22_analysis_history_detail_ui_design.md](../docs/22_analysis_history_detail_ui_design.md)、上記「frontend履歴詳細UI設計」参照）まで完了しているが実装はまだ行っていない。`analysisRunId`のAPIレスポンス追加・pgvector導入・非同期job化・観測系の個別テーブル化・認証/RLSもいずれもまだ行っていない）
