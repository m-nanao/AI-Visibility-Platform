...
def test_fetch_posts_to_the_responses_api_url(monkeypatch):
    ...
    fetch_chatgpt_observation(_CREDENTIALS, "Acme", model="gpt-5-mini", max_output_tokens=700)
...
def test_fetch_system_prompt_forbids_browsing(monkeypatch):
    ...
    assert "Do not browse the web" in system_content
...
def test_fetch_extracts_output_text_field_when_present(monkeypatch):
    ...
    result = fetch_chatgpt_observation(_CREDENTIALS, "Acme", model="gpt-5-mini", max_output_tokens=700)
