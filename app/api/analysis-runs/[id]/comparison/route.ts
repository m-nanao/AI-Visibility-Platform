 ...
 * mirroring app/api/analysis-runs/[id]/route.ts's PYTHON_ANALYSIS_API_URL
 * usage and error-shape conventions exactly (see
 * docs/25_analysis_history_comparison_design.md).
 *
 * Response shapes returned to the caller:
 * - 200: the Python API's AnalysisRunComparisonResponse, validated with
 *   Zod. `previous`/`diff` may both be null (no earlier run for this
 *   brand yet) — that is a normal 200, not an error.
 * - 503: history read is disabled or unconfigured — same meaning as
 *   the other two read endpoints (see app/api/analysis-runs/route.ts).
 ...
 *   the Python API (backend/main.py's GET /analysis-runs/{id}/comparison).
 * - 502: the Python API responded with something else unexpected
 *   (non-2xx other than 403/404/503, invalid JSON, or a body that
...
export async function GET(
  ...
  try {
    response = await fetch(
      `${baseUrl.replace(/\/$/, "")}/analysis-runs/${encodeURIComponent(id)}/comparison`,
      { headers },
    );
  } catch {
    console.warn("[analysis-runs/[id]/comparison] Python API request failed");
    return NextResponse.json(
      { error: "前回比較を読み込めませんでした。" },
      { status: 503 },
    );
  }
  ...
  if (!response.ok) {
    console.warn(
      `[analysis-runs/[id]/comparison] Python API returned HTTP ${response.status}`,
    );
    return NextResponse.json(
      { error: "前回比較を読み込めませんでした。" },
      { status: 502 },
  ...
  try {
    ...
    console.warn("[analysis-runs/[id]/comparison] Python API returned invalid JSON");
    return NextResponse.json(
      { error: "前回比較を読み込めませんでした。" },
      { status: 502 },
    );
  ...
  if (!parsed.success) {
    console.warn(
      `[analysis-runs/[id]/comparison] Python API response failed schema validation (${parsed.reason})`,
    );
    return NextResponse.json(
      { error: "前回比較を読み込めませんでした。" },
      { status: 502 },
