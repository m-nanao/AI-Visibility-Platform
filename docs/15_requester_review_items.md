...
## 9. MVPレビュー前の表示文言最終確認（2026-09-18追記）

上記の承認済み方針（断定表現は避ける等）に沿って、MVPレビュー直前に画面表示文言全体を再点検した（`chore/mvp-review-copy-final-check`）。新たに依頼者確認を要する表現方針の変更ではなく、**承認済み方針と実装内容（`visibilityScore`はAI/LLM呼び出しなしの推定値である等）の間にできていた食い違いを、既存方針に沿って修正した内部整合性の是正**である。具体的な変更内容は[36_multi_ai_comparison_design.md](./36_multi_ai_comparison_design.md)「15. MVPレビュー前の表示文言最終確認」を参照。
