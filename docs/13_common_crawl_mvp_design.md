...
- **上記の方針は2026-08-20に依頼者から承認済み**。[15_requester_review_items.md](./15_requester_review_items.md)「依頼者確認結果」のとおり、「Common Crawlは補助データ」「AI学習内容そのものを保証しない」「未取得時も通常分析を継続する」「次フェーズはDB保存/非同期化/定期取得/履歴管理を優先候補とする」等の方針・トーン・優先順位は確定した前提として扱ってよい。個別の表示名・説明文の最終文言（本ドキュメント11章、[15_requester_review_items.md](./15_requester_review_items.md)2章の「候補」）も同日`style/finalize-approved-ui-copy`で確定済み。
