...
- 認証・レート制限・監視はまだ本格導入していない。確認用途が長期化する場合は、簡易パスコードまたは正式な認証の導入を検討する（今回は導入しない。[05_tasks.md](./05_tasks.md) Phase 4.5参照）。
- 将来的には独自ドメイン環境へ移行する予定（上記「環境の位置づけ」参照）。
...
3. **ブラウザでの確認**
   ...
   <https://ai-visibility-platform-eight.vercel.app/> を開き、ブランド名を入力して「分析開始」を押し、5セクションの結果が表示されることを確認する。画面上部の要約バッジ（例:「共起語のみ実計算、その他は開発用データ」）が表示されていることも確認する。
...
- **認証・アクセス制限（URLを知っていれば誰でも操作できる）**。上記2つの公開URLはこのドキュメント・リポジトリに記載されているため、実質的に非公開情報ではない。
- レート制限・利用量監視
- 永続化（PostgreSQL未接続のため、分析結果は保存されず再読み込みで消える）
- robots.txt確認・アクセス負荷対策（`urls`機能。[backend/README.md](../backend/README.md)の「運用上の注意」参照）

**確認作業が終わったら、公開を停止するか認証を追加するかを判断すること。** このままアクセス制限なしで放置しない（[05_tasks.md](./05_tasks.md) のPhase 4.5に未完了タスクとして記録済み）。停止方法は[8. ロールバック方法](#8-ロールバック方法)の「完全に公開を止めたい場合」を参照。
