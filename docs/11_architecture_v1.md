...
`web_fetcher.py`は`document_cleaner.clean_html_to_text()`の戻り値に対して、`backend/main.py`は`user_provided`の`documents`各要素に対して、それぞれ`normalize_text()`を適用してから`Document.text`に格納する。development sample文章（`documentsSource: "development_sample"`）はまだ`Document[]`化されておらず対象外。
...
`Document`型の定義、Provider（`web_fetcher.py`）とCleaner（`document_cleaner.py`）の分離、Normalizer（`document_normalizer.py`）、Analyzer側のアダプター（`compute_cooccurrence_ranking_from_documents()`）は実装済み。残るギャップは、Chunkerという独立した処理段階がまだないこと。development sample文章（`documentsSource: "development_sample"`）も`Document[]`化されていない（`DocumentSourceType`に対応する値がないため、Normalizerの対象にも含まれていない）。5章「未実装」およびPhase 4.2（[05_tasks.md](./05_tasks.md)）に記載の通り、共起語抽出の精度向上に着手する前に、Chunkerを追加してこの5段階のパイプラインを完成させることを推奨する（10章参照）。
...
- Document Normalizer（`backend/services/document_normalizer.py`）としてUnicode・空白・不可視文字の正規化を実装。`normalize_text()`。`web_fetch`（Cleaner出力）・`user_provided`（`documents`各要素）の両方に適用
...
`Document`型の定義とProviderレベルの変換（`user_provided`/`web_fetch`→`Document[]`）、Cleanerの分離（`document_cleaner.py`）、Normalizerの追加（`document_normalizer.py`、2026-07-16）、Analyzerアダプター（`compute_cooccurrence_ranking_from_documents()`）は実装済み。残作業は以下（着手時は[10_ai_development_workflow.md](./10_ai_development_workflow.md)の「6. 1タスクの粒度」に従い1タスクずつに分解する）。

1. Chunker（長文分割）を独立した処理として追加する
2. development sample文章を`Document[]`化するか、`DocumentSourceType`に対応する値を追加するか判断する（4章参照）
3. `Document.sourceType`と`meta.documentsSource`を統合するか、別概念として維持するか判断する（4章参照）

理由: 今後Common CrawlやDataForSEOを入れる前に、取得元を`Document[]`に統一する設計へ整えるため。Provider・Cleaner・Normalizerの分離は完了したが、Chunkerが独立した段階になっていない状態でCommon Crawl（7章）やDataForSEO（6章）を追加すると、Normalizer以降の処理が取得元ごとに再度分岐しかねない。先にPipelineの残り1段階を整理しておくことで、C〜Eのタスクを見通しよく進められる。
