...
共起解析、文脈分析、センチメント、知識グラフ、改善提案を行う。現状は共起解析（`backend/services/cooccurrence.py`）と、軽量な文脈分析（`backend/services/context_analysis.py`、2026-07-16、後述）が実装済み。他は`backend/services/mock_analysis.py`による固定データ（5章参照）。
...
**Document Pipelineの5段階（Provider→Cleaner→Normalizer→Chunker→Analyzer）は2026-07-16時点で一通り実装済み。「C. 文脈分析の実装」も軽量版（context-analysis-lite）が2026-07-16に完了した。次に進む推奨は「A. 実URL分析の品質検証」、または残りのmockセクション（ブランド認知サマリー・AI Overview比較・改善提案）への着手とする。**
