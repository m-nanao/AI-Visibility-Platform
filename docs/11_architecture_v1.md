...
`Document`型の定義とProviderレベルの変換（`user_provided`/`web_fetch`→`Document[]`）、Cleanerの分離（`document_cleaner.py`）、Analyzerアダプター（`compute_cooccurrence_ranking_from_documents()`）は実装済み。残作業は以下（着手時は[10_ai_development_workflow.md](./10_ai_development_workflow.md)の「6. 1タスクの粒度」に従い1タスクずつに分解する）。

1. Normalizer（全角半角・空白等の本格的な正規化）を独立した処理として追加する（Cleanerは最小限の空白圧縮のみ）
2. Chunker（長文分割）を独立した処理として追加する
3. development sample文章を`Document[]`化するか、`DocumentSourceType`に対応する値を追加するか判断する（4章参照）
4. `Document.sourceType`と`meta.documentsSource`を統合するか、別概念として維持するか判断する（4章参照）

理由: 今後Common CrawlやDataForSEOを入れる前に、取得元を`Document[]`に統一する設計へ整えるため。Provider・Cleanerの分離は完了したが、Normalizer/Chunkerが独立した段階になっていない状態でCommon Crawl（7章）やDataForSEO（6章）を追加すると、Cleaner以降の処理が取得元ごとに再度分岐しかねない。先にPipelineの残り2段階を整理しておくことで、C〜Eのタスクを見通しよく進められる。
