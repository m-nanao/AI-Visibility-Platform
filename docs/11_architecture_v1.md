...
- `common_crawl` / `dataforseo`: Document Providerとしては未実装（7章・6章参照）。**2026-07-17に実装したDataForSEO Sandbox接続、および2026-07-23に追加したLive手動確認用接続（`dataforseo_client.py`）はこのDocument Pipelineには合流せず、`aiOverviewComparison`セクション専用の別経路であることに注意**（4章「AI Overview比較のprovider mode」参照）。**同じく2026-07-23に追加したChatGPT相当モデルの1問観測（`chatgpt_provider.py`）も、`brandName`をOpenAI APIへ直接質問するだけでDocument化・Document Pipelineへの合流は一切行わない別経路である**
...
- **レスポンス変換**: `ai_overview`タイプの項目が見つかった場合のみ`AIOverviewComparisonItem`（`mentioned`/`rank`/`summary`、生データ全文は含めない）へ変換し`"real"`を返す。`rank`は`rank_absolute`優先・`rank_group`フォールバック、`summary`は`markdown`優先・`text`フォールバックで作成（markdownの画像・リンク記法は軽く平文化）。`mentioned`判定には`markdown`/`text`に加え入れ子`items[]`・`references[].title/.text/.domain`も使うが、`references`自体は`summary`には含めない。見つからない・想定外の形の場合は`[]`・`"unavailable"`にフォールバックする（`success`は「HTTP成功」ではなく「AI Overview項目を発見」を意味する設計。詳細は[07_decisions.md](./07_decisions.md)参照）。`platform`ラベルはSandbox/Liveで`"Google AI Mode (DataForSEO Sandbox)"`/`"Google AI Mode (DataForSEO Live)"`と区別する。**2026-07-23追加**: `summary`と同じ元テキストからより長い抜粋`fullSummary`（最大2500文字、段落区切りは残す）を、`references[]`/`links[]`（自身とネストされた`items[]`のもの）から引用元一覧`references`（`title`/`domain`/`url`/`text`/`source`/`position`、urlまたはdomain+titleで重複排除・最大10件）も抽出して`AIOverviewComparisonItem`に含めるようになった（いずれも任意フィールド、既存の`mentioned`/`rank`/`summary`は変更なし）。`ai_overview_provider.py`はさらに、リクエストの`urls`から抽出したドメインが`references`と一致するかを比較する`ownDomainReferenced`（単純な文字列比較、`urls`未指定時は`None`）も付与する。**DataForSEOレスポンスの生データ全文はこれらの追加後も一切返さない**。

**参照元の簡易分類（2026-07-23追加）**: `ai_overview_provider.py`の`_classify_reference_category()`が各`references[]`要素にルールベースの`category`（`"official"`/`"wikipedia"`/`"sns"`/`"ugc"`/`"news"`/`"media"`/`"video"`/`"other"`）を付与する。判定は新たなDataForSEO呼び出しを伴わず、既存の`references`とリクエストの`urls`だけから行う——自社ドメイン一致（サブドメイン含む）が最優先で`"official"`、それ以外は小さなハードコードdomainリスト（Wikipedia/SNS/UGC/動画/ニュース）との照合、いずれにも一致しなければ`"other"`（`"media"`は将来のより正確な判定用に予約した値で、現時点では何も分類されない）。`_build_reference_summary()`がこれを`referenceSummary`（`{total, official, thirdParty, categories}`）へ集計する。`build_improvement_suggestions()`（`improvement_suggestions.py`）にも`ai_overview_items`引数を追加し、`ownDomainReferenced`/`referenceSummary`から改善提案を最大1件（自社サイト未参照/参照済み/第三者依存の3パターン、排他的）追加するようになった——これに合わせて`main.py`の実行順序を、`aiOverviewComparison`計算後に`improvements`を計算するよう入れ替えた（以前は逆順）。
...
**ChatGPT相当モデルの1問観測（`chatgpt_settings.py` / `chatgpt_client.py` / `chatgpt_provider.py`、2026-07-23新設）**: `aiOverviewComparison`へ、OpenAI APIのモデルにブランド名を1問だけ質問した回答を追加する仕組み。**ChatGPTアプリ画面そのものの内部認識を再現するものではなく**、OpenAI APIのモデルへの1回の質問とその回答を「ChatGPT相当モデルの観測結果」として扱う点に注意（Web検索・参照元付き回答は使わない）。DataForSEOの`ai_overview_provider.py`/`dataforseo_client.py`/`dataforseo_settings.py`の三層分割と意図的に同じ構造を踏襲している。

- `chatgpt_settings.py`はOpenAI固有の設定（`OPENAI_API_KEY`/`CHATGPT_MODEL`（デフォルト`gpt-5-mini`）/`CHATGPT_MAX_OUTPUT_TOKENS`（デフォルト700、100〜1500へクランプ）/`CHATGPT_REQUEST_LIMIT_PER_ANALYZE`（デフォルト1、不正な整数値のみデフォルトへフォールバックし、有効だが1以外の整数はそのまま読み通す設計））のみを扱う。`ChatGptCredentials.__repr__`はAPIキーを`<redacted>`にする（`DataForSEOCredentials`と同様）。モード判定ロジックは一切持たない。
- `chatgpt_provider.py`が`CHATGPT_PROVIDER_MODE`環境変数（デフォルト`off`）と`ALLOW_CHATGPT_MODE_OVERRIDE`環境変数（デフォルト`false`）による2段階ゲートを判定する（`resolve_chatgpt_mode()`）。加えて`build_chatgpt_observation()`が、モードが`"openai"`でも`OPENAI_API_KEY`未設定・`request_limit_per_analyze != 1`のいずれかであればOpenAI APIを一切呼ばず`"unavailable"`を返す。
- `chatgpt_client.py`の`fetch_chatgpt_observation()`は`httpx`で`POST https://api.openai.com/v1/responses`へ直接REST接続する（`openai` SDKは追加していない）。`store: false`を指定し、レスポンスは`output_text`を優先、なければ`output[].content[].text`を走査して抽出する。ゲート判定ロジックは持たず、呼び出し元（`chatgpt_provider.py`）が渡した引数をそのまま使う——DataForSEOの`dataforseo_client.py`と同じ「クライアントはゲートを知らない」設計。
- 成功時は既存の`AIOverviewComparisonItem`型をそのまま再利用し、`platform: "ChatGPT (OpenAI API)"`、`rank`/`references`/`referenceSummary`/`ownDomainReferenced`はすべて`None`固定で1件を`aiOverviewComparison`へ追加する。これらのフィールドは元々すべて任意だったため、`AIOverviewComparisonSection.tsx`/`meta-label.ts`の変更は不要だった。
- **mockモードとの重複回避**: `aiOverviewMode`（`ai_overview_provider.py`側の解決結果）が`"mock"`の場合、`main.py`はChatGPT観測を呼び出す前にスキップする。既存の固定`aiOverviewComparison`フィクスチャに元から「ChatGPT」という名前のダミーカードが含まれているため、実データのカードと重複させないための分岐であり、ポストホックな重複排除は行わない。
- `meta.chatgptProvider`（`{mode, status, reason, environment}`、`status: "real"|"off"|"unavailable"`、`environment: "api"|"off"|"unavailable"`）は`meta.aiOverviewProvider`と完全に独立して計算・付与される。APIキー・認証情報は`reason`・ログ・レスポンスのいずれにも一切含めない。
- 検証用に`NEXT_PUBLIC_ENABLE_CHATGPT_MODE_SELECTOR=true`のときのみ表示されるフロントの検証用selectを追加（`BrandInputForm.tsx`）。`chatgptMode`はUI表示時のみリクエストボディへ含め、非表示時は省略する（既存のAI Overviewモード検証用selectと同じパターン）。
- 1 `/analyze`あたり最大1回のOpenAI API呼び出しに固定（`CHATGPT_REQUEST_LIMIT_PER_ANALYZE != 1`はゲート不成立として扱う）。複数質問・複数ブランド同時処理・Web検索ツール利用・参照元付き回答は対象外。詳細は[07_decisions.md](./07_decisions.md)「2026-07-23 — ChatGPT相当モデルの1問観測を...」参照。
...
- ChatGPT相当モデルの1問観測（`backend/services/chatgpt_settings.py`/`chatgpt_client.py`/`chatgpt_provider.py`、2026-07-23新設）として、OpenAI APIへブランド名を1問質問し回答を`aiOverviewComparison`へ追加する仕組みを導入。`CHATGPT_PROVIDER_MODE`（デフォルト`off`）・`ALLOW_CHATGPT_MODE_OVERRIDE`（デフォルト`false`）の2段階ゲートに加え、`OPENAI_API_KEY`設定済み・`CHATGPT_REQUEST_LIMIT_PER_ANALYZE==1`が揃わない限りOpenAI APIを呼ばない。`aiOverviewMode=="mock"`のときは既存の固定`ChatGPT`ダミーカードとの重複を避けるため常にスキップする。1 `/analyze`あたり最大1回のOpenAI API呼び出しに固定。`meta.chatgptProvider`（`{mode, status, reason, environment}`）で状態を報告する
...
- ChatGPT相当モデル観測の常時運用化、複数質問対応、Claude/Gemini等の他モデルへの拡張、Web検索ツールを使った参照元付き回答（1問限りの単純観測（`chatgpt_provider.py`）はデフォルトoffの検証用途として実装済み）
...
- I. ChatGPT相当モデル観測の拡張（1問限りの検証用実装（`chatgpt_provider.py`）は完了。常時運用化・複数質問・Claude/Gemini等の他モデルへの拡張はこれから）

**Document Pipelineの5段階（Provider→Cleaner→Normalizer→Chunker→Analyzer）は2026-07-16時点で一通り実装済み。「C. 文脈分析の実装」「F. ブランド認知サマリーの実装」「G. 改善提案の実装」もいずれも軽量版（context-analysis-lite / brand-summary-lite / improvement-suggestions-lite）が2026-07-16に完了した。「H. AI Overview比較のDataForSEO本接続」は2026-07-17にprovider切り替え基盤（mock/off/dataforseoモード）・認証情報/実行安全ルールの設計（`dataforseo_settings.py`）・DataForSEO **Sandbox**への実接続（`dataforseo_client.py`）が完了し、2026-07-23には5つの手動確認用ゲートが揃った場合に限る**Live**本番API接続、さらに同日中にAI Overview本文詳細（`fullSummary`）・引用元一覧（`references`）・自社ドメイン参照判定（`ownDomainReferenced`）の追加、続けて参照元のルールベース簡易分類（`references[].category`/`referenceSummary`）と、それを改善提案に軽く反映する仕組みも完了した。同じく2026-07-23、「I. ChatGPT相当モデル観測の拡張」の第一歩として、OpenAI APIへの1問観測（`chatgpt_settings.py`/`chatgpt_client.py`/`chatgpt_provider.py`）をデフォルトoffの検証用途として追加し、`aiOverviewComparison`へ「ChatGPT (OpenAI API)」カードを条件付きで追加できるようにした。次に進む推奨は「A. 実URL分析の品質検証」、または「D. DataForSEO調査・PoC」を経て、Liveの常時運用化（費用管理・複数キーワード対応等）に着手することとする。**
