...
- `user_provided`: `documents`パラメータでそのまま受け取る（Provider相当の処理はほぼ不要）
- `web_fetch`: `backend/services/web_fetcher.py`（SSRF対策込み、同時実行数3で並列取得。詳細は[07_decisions.md](./07_decisions.md)の「SSRF対策は...」参照）
...
現在の実装は、Provider（`web_fetcher.py`）とCleaner（同ファイル内のBeautifulSoup処理）が未分離のまま1つのモジュールに同居しており、Normalizer・Chunkerという独立した処理段階もまだない。5章「未実装」およびPhase 4.2（[05_tasks.md](./05_tasks.md)）に記載の通り、共起語抽出の精度向上に着手する前に、この5段階のパイプラインへ整理し直すことを推奨する（10章参照）。
...
**次に進む推奨は「B. Document Pipelineのリファクタリング」とする。**

理由: 今後Common CrawlやDataForSEOを入れる前に、取得元を`Document[]`に統一する設計へ整えるため。現状（4章「現状とのギャップ」参照）はProviderとCleanerが`web_fetcher.py`内に未分離のまま同居しており、この状態のままCommon Crawl（7章）やDataForSEO（6章）を追加すると、取得元ごとに個別の処理が増殖し、Analyzer側のコードが取得元を意識せざるを得なくなる。先にPipelineの5段階（Provider/Cleaner/Normalizer/Chunker/Analyzer）へ整理しておくことで、C〜Eのタスクを見通しよく進められる。

着手時は、[10_ai_development_workflow.md](./10_ai_development_workflow.md)の「6. 1タスクの粒度」に従い、例えば「①`web_fetcher.py`からCleanerを分離する」「②`Document`型を`app/lib/types.ts`・`backend/models.py`に定義する」のように1タスクずつに分解してから着手する。
