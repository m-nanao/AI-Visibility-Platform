...
実装: [backend/main.py](../backend/main.py)（ルート定義）、[backend/models.py](../backend/models.py)（Pydanticモデル）、[backend/services/mock_analysis.py](../backend/services/mock_analysis.py)（ダミーデータ生成）、[backend/services/cooccurrence.py](../backend/services/cooccurrence.py)（共起語抽出の実計算）、[backend/services/context_analysis.py](../backend/services/context_analysis.py)（文脈分析の実計算、キーワードベースの軽量版）、[backend/services/brand_summary.py](../backend/services/brand_summary.py)（ブランド認知サマリーの実計算、ルールベース・テンプレート生成の軽量版）、[backend/services/improvement_suggestions.py](../backend/services/improvement_suggestions.py)（改善提案の実計算、ルールベースの軽量版）、[backend/services/ai_overview_provider.py](../backend/services/ai_overview_provider.py)（AI Overview比較のprovider切り替え、mock/off/dataforseo）、[backend/services/dataforseo_settings.py](../backend/services/dataforseo_settings.py)（DataForSEO認証情報・実行安全ルールの設定読み取り。外部API通信はしない）、[backend/services/dataforseo_client.py](../backend/services/dataforseo_client.py)（DataForSEO **Sandbox**への実接続。Liveへも手動確認用ゲートがすべて揃った場合のみ接続）、[backend/services/sample_documents.py](../backend/services/sample_documents.py)（開発用サンプル文章）、[backend/services/web_fetcher.py](../backend/services/web_fetcher.py)（URLから本文取得）。起動方法は [backend/README.md](../backend/README.md)。
...
- `aiOverviewMode`: 任意（`"mock" | "off" | "dataforseo"`）。`aiOverviewComparison`のprovider modeをリクエスト単位で上書きする。Python API側で`ALLOW_AI_OVERVIEW_MODE_OVERRIDE=true`が設定されていない限り無視される（詳細は下記「AI Overview比較のprovider mode」参照）。上記3値以外を渡すと400になる。
...
- `aiOverviewComparison` は `services/ai_overview_provider.py` のprovider mode（`mock`/`off`/`dataforseo`）で切り替わる。デフォルトの`mock`では固定データを返す。`dataforseo`モードはDataForSEO **Sandbox**への接続を実装済みで、**Live本番APIへの接続も、5つの手動確認用ゲートがすべて揃った場合に限り実装済み**（常時運用ではない、[05_tasks.md](./05_tasks.md) Phase 4.2参照）。この状態は `meta.sections` に反映される（`cooccurrenceRanking` / `contextAnalysis` / `summary` / `improvements` の4つが `"real"`、`aiOverviewComparison` は `"mock"`・Sandbox/Live接続成功時のみ`"real"`・それ以外は`"unavailable"`）。詳細は上記「AI Overview比較のprovider mode」を参照。
