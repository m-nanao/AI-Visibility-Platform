...
| フィールド | 型 | 説明 |
| --- | --- | --- |
| `meta.source` | `"python_mock" \| "nextjs_mock" \| "real_analysis"` | データがどこで生成されたか。`real_analysis` は将来、実データ分析が実装された際に使う値（現時点ではどちらのサービスも生成しない） |
| `meta.isMock` | `boolean` | 固定・ダミーデータかどうか |
| `meta.generatedAt` | `string` | 生成日時（ISO 8601） |
...
実装: [backend/main.py](../backend/main.py)（ルート定義）、[backend/models.py](../backend/models.py)（Pydanticモデル）、[backend/services/mock_analysis.py](../backend/services/mock_analysis.py)（ダミーデータ生成）。起動方法は [backend/README.md](../backend/README.md)。
...
```json
{
  ...
  "meta": {
    "source": "python_mock",
    "isMock": true,
    "generatedAt": "2026-07-10T00:00:00.000000+00:00"
  }
}
```
...
- 現時点では `services/mock_analysis.py` の `build_dummy_analysis()` が生成する固定データを返すのみ。Common Crawl / DataForSEO / DB接続は行っていない。`meta.source` は常に `"python_mock"`。
