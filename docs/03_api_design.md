...
`aiOverviewComparison`は、他の4セクションとは独立したprovider切り替え基盤（`backend/services/ai_overview_provider.py`）を持つ。`dataforseo`モードはDataForSEO **Sandbox**への実際の接続を実装済み（`backend/services/dataforseo_client.py`）——ただし**Live APIへの接続は今回も対象外**で、`DATAFORSEO_API_ENV=live`の場合は常に`"unavailable"`を返す（費用が発生し得るため意図的に除外）。

- **mode**: `"mock"`（デフォルト、固定データ4件、`meta.sections.aiOverviewComparison: "mock"`）/ `"off"`（セクション無効化、`[]`、`"unavailable"`）/ `"dataforseo"`（`DATAFORSEO_API_ENV=sandbox`かつ認証情報設定済みの場合のみDataForSEO Sandboxへ実際に接続。成功すれば1件のデータ・`"real"`、それ以外（認証情報未設定・`live`指定・Sandbox接続失敗）は`[]`・`"unavailable"`）。
...
### DataForSEO認証情報・実行安全ルール / Sandbox接続

認証情報の扱い方と誤実行防止ルール（`backend/services/dataforseo_settings.py`）、および実際にDataForSEO Sandboxへ接続するクライアント（`backend/services/dataforseo_client.py`）。**Live APIへの接続はこのプロジェクトではまだ実装しておらず、`DATAFORSEO_API_ENV=live`の場合は常に外部API通信を行わない。**
...
- **`password`は`DataForSEOSettings`には実値を保持しない**: 読み取った瞬間に「設定されているかどうか」の真偽値へ変換し、実際の文字列はどこにも残らない。ログ・APIレスポンス・`repr()`いずれにも露出しようがない設計。Sandbox接続のBasic Auth構築にだけ必要な実値は、別型の`DataForSEOCredentials`が保持し、リクエスト構築の直前でのみ使い保存・ログ出力しない。
- **Sandbox/Live切り替え**: `DATAFORSEO_API_ENV`（`sandbox`（デフォルト）/`live`）。`sandbox`かつ認証情報設定済みの場合のみ、実際にDataForSEO Sandbox（`https://sandbox.dataforseo.com`）へHTTP接続する。
- **Live API誤有効化の防止**: `DATAFORSEO_LIVE_API_ENABLED`（デフォルト`false`）が`true`で、かつ`DATAFORSEO_API_ENV=live`で、かつ認証情報が設定されている——**3条件すべて**が揃っても、`ai_overview_provider.py`が`api_env == "live"`を無条件で拒否するため、このプロジェクトでは今回もLive APIは一切呼ばれない。
- **費用の急増防止**: `DATAFORSEO_REQUEST_LIMIT_PER_ANALYZE`（デフォルト`1`、上限`10`）で1回の`/analyze`あたりのDataForSEOリクエスト数に上限を設ける（Sandbox接続は常に1リクエストのみ発行するため、この値自体は現状の実装では参照されていない。将来の複数キーワード対応向けの受け皿）。
- **DataForSEOの「Standard」と「Live」というリクエスト方式の違い**: DataForSEO SERP APIには`task_post`/`task_get`による非同期のStandard方式と、即時レスポンスのLive方式（DataForSEO独自の呼称、このプロジェクトが区別しているSandbox/Live**環境**とは別の軸）がある。今回もいずれのエンドポイントも即時レスポンスのLive方式のみを、Sandbox**環境**に対して呼び出す。Standard方式（非同期タスク管理）は対象外。
- **エンドポイント選定（`DATAFORSEO_SERP_ENDPOINT`、デフォルト`google_ai_mode_live_advanced`）**: DataForSEO Sandboxに対して手動で「Vercel」を`location_code=2392`（日本）・`language_code=ja`・`device=desktop`・`os=windows`で検索したところ、`/v3/serp/google/ai_mode/live/advanced`（Google AI Mode向けエンドポイント）は`item_types: ["ai_overview"]`・`markdown`・`references`を含む結果を確実に返したのに対し、`/v3/serp/google/organic/live/advanced`（Google Organic SERP向け）は`ai_overview`項目を確実には返さなかった。このため標準エンドポイントを`google_ai_mode_live_advanced`に変更した（`google_organic_live_advanced`は旧実装との互換用に選択可能なまま残している）。**Google AI OverviewとGoogle AI Modeは別の機能・製品**であり、この実装はDataForSEOの`ai_mode`エンドポイントが返す`ai_overview`タイプの項目を、このMVPの比較目的においては同等に扱っている。
- **リクエストパラメータ**: `location_code`/`language_code`/`device`/`os`はいずれも`DATAFORSEO_LOCATION_CODE`/`DATAFORSEO_LANGUAGE_CODE`/`DATAFORSEO_DEVICE`/`DATAFORSEO_OS`で上書き可能。デフォルトは上記の手動検証で成功した組み合わせ。
- `aiOverviewMode: "dataforseo"`の`meta.aiOverviewProvider.reason`には、上記の設定状態・Sandbox接続結果に基づく安全な説明文（例:「credentials are not configured」「Live API is not implemented in this task」「DataForSEO Sandbox AI Mode request succeeded」「no ai_overview item was found. endpoint=...」等）が入るが、`login`/`password`の値そのものは絶対に含まれない。
- **DataForSEO Sandboxのレスポンスは実際の本番SERPデータではない**（テスト用モックデータであり、実際にGoogleがそのクエリに対してAI Overviewを表示するかどうかを表さない）。

詳細は[backend/README.md](../backend/README.md)の「DataForSEO設定（`dataforseo_settings.py`）」「DataForSEO Sandbox接続（`dataforseo_client.py`）」、運用方針の経緯は[07_decisions.md](./07_decisions.md)を参照。
...
実装: [backend/main.py](../backend/main.py)（ルート定義）、[backend/models.py](../backend/models.py)（Pydanticモデル）、[backend/services/mock_analysis.py](../backend/services/mock_analysis.py)（ダミーデータ生成）、[backend/services/cooccurrence.py](../backend/services/cooccurrence.py)（共起語抽出の実計算）、[backend/services/context_analysis.py](../backend/services/context_analysis.py)（文脈分析の実計算、キーワードベースの軽量版）、[backend/services/brand_summary.py](../backend/services/brand_summary.py)（ブランド認知サマリーの実計算、ルールベース・テンプレート生成の軽量版）、[backend/services/improvement_suggestions.py](../backend/services/improvement_suggestions.py)（改善提案の実計算、ルールベースの軽量版）、[backend/services/ai_overview_provider.py](../backend/services/ai_overview_provider.py)（AI Overview比較のprovider切り替え、mock/off/dataforseo）、[backend/services/dataforseo_settings.py](../backend/services/dataforseo_settings.py)（DataForSEO認証情報・実行安全ルールの設定読み取り。外部API通信はしない）、[backend/services/dataforseo_client.py](../backend/services/dataforseo_client.py)（DataForSEO **Sandbox**への実接続。Liveへは接続しない）、[backend/services/sample_documents.py](../backend/services/sample_documents.py)（開発用サンプル文章）、[backend/services/web_fetcher.py](../backend/services/web_fetcher.py)（URLから本文取得）。起動方法は [backend/README.md](../backend/README.md)。
...
- `aiOverviewComparison` は `services/ai_overview_provider.py` のprovider mode（`mock`/`off`/`dataforseo`）で切り替わる。デフォルトの`mock`では固定データを返す。`dataforseo`モードはDataForSEO **Sandbox**への接続を実装済みだが、**Live APIへの接続はまだ未実装**（[05_tasks.md](./05_tasks.md) Phase 4.2参照）。この状態は `meta.sections` に反映される（`cooccurrenceRanking` / `contextAnalysis` / `summary` / `improvements` の4つが `"real"`、`aiOverviewComparison` は `"mock"`・Sandbox接続成功時のみ`"real"`・それ以外は`"unavailable"`）。詳細は上記「AI Overview比較のprovider mode」を参照。
