...
- **DataForSEOの「Standard」と「Live」というリクエスト方式の違い**: DataForSEO SERP APIには`task_post`/`task_get`による非同期のStandard方式と、即時レスポンスのLive方式（DataForSEO独自の呼称、このプロジェクトが区別しているSandbox/Live**環境**とは別の軸）がある。今回は即時レスポンスのLive方式（`/v3/serp/google/organic/live/advanced`）のみを、Sandbox**環境**に対して呼び出す。Standard方式（非同期タスク管理）は対象外。
- **エンドポイント選定の理由**: Google Organic SERPのレスポンスは、Googleがそのクエリに対してAI Overviewを表示している場合、`items[]`内に`ai_overview`タイプの項目として含まれる（追加課金なし）。より高額な`/v3/serp/google/ai_mode/live/advanced`（Google AI Mode、AI Overviewとは別の製品）は意図的に呼ばない。**Google AI OverviewとGoogle AI Modeが同一のSandboxレスポンス構造で表現されるかは未検証**（この開発環境から実APIへアクセスできないため）。
- `aiOverviewMode: "dataforseo"`の`meta.aiOverviewProvider.reason`には、上記の設定状態・Sandbox接続結果に基づく安全な説明文（例:「credentials are not configured」「Live API is not implemented in this task」「DataForSEO Sandbox request succeeded」等）が入るが、`login`/`password`の値そのものは絶対に含まれない。
