...
`aiOverviewComparison`は、他の4セクションとは独立したprovider切り替え基盤（`backend/services/ai_overview_provider.py`）を持つ。DataForSEO本接続はまだ行っておらず、テスト中の意図しない実API実行（費用発生の可能性）を防ぐため、本接続に先立って切り替え基盤のみを導入した。

- **mode**: `"mock"`（デフォルト、固定データ4件、`meta.sections.aiOverviewComparison: "mock"`）/ `"off"`（セクション無効化、`[]`、`"unavailable"`）/ `"dataforseo"`（**今回は未実装**、外部APIは呼ばず`[]`、`"unavailable"`）。
...
- `aiOverviewComparison` は `services/ai_overview_provider.py` のprovider mode（`mock`/`off`/`dataforseo`）で切り替わる。デフォルトの`mock`では固定データを返す。DataForSEO本接続はまだ未実装（[05_tasks.md](./05_tasks.md) Phase 4.2参照）。この状態は `meta.sections` に反映される（`cooccurrenceRanking` / `contextAnalysis` / `summary` / `improvements` の4つが `"real"`、`aiOverviewComparison` は `"mock"` または `"unavailable"`）。詳細は上記「AI Overview比較のprovider mode」を参照。
