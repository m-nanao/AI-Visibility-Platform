...
最小MVPの設計を[13_common_crawl_mvp_design.md](./13_common_crawl_mvp_design.md)としてまとめた（2026-07-28）。ブランド名の全文検索ではなく、**domain指定でのCommon Crawl Index検索から始める**方針。設定モジュールとIndex API検索クライアントを実装済み（2026-07-28、`backend/services/common_crawl_settings.py`/`backend/services/common_crawl_index.py`。詳細は[backend/README.md](../backend/README.md)「Common Crawl最小連携」）。**WARC本文取得・`Document[]`統合・`/analyze`統合・UI連携はまだ未着手。**
...
- **Next（次のステップ、優先順）**:
  ...
  - `CommonCrawlCandidate` → `Document[]`統合（既存のDocument Pipelineへ合流、`sourceType: "common_crawl"`）
...
| 3 | Common Crawl / DataForSEO連携 | DataForSEOはSandbox/Live接続まで実装済み（[11_architecture_v1.md](./11_architecture_v1.md)参照）。Common Crawlは設計（[13_common_crawl_mvp_design.md](./13_common_crawl_mvp_design.md)）＋settings/Index API client実装済み（2026-07-28）。WARC取得/`Document[]`統合/`/analyze`統合/UIは未着手 |
