...
- [x] AI Overview比較カードの詳細表示を「続きを見る」に変更し、summaryとfullSummaryの重複表示を避ける（`style/ai-overview-continuation-display`、2026-07-28）。ChatGPT観測機能の追加で、回答が短めの場合に「概要」（summary）と「詳細を見る」（fullSummary全文）の内容がほぼ同じに見える問題が見つかったための表示改善。backend/API/DataForSEO/OpenAI providerは一切変更せず、UI表示用helper（`app/lib/meta-label.ts`）のみで対応した。`getAiOverviewItemDetailDisplay()`の`hasDetail`/`detailText`を`hasContinuation`/`continuationText`にリネームし、新設の`buildContinuationText(summary, fullSummary)`がsummaryとfullSummaryの共通の先頭部分を取り除いた続き部分だけを抽出する（空白・改行の差分に強い比較のため、比較時のみ連続空白を1つに・改行を空白に正規化し、summary末尾の`…`/`...`を除去してから前方一致を判定。一致した場合は元のfullSummary側の対応する位置までを正規化文字数から逆算してマッピングし、改行を保ったまま続き部分を切り出す）。続き部分が30文字未満、またはfullSummaryがsummaryとほぼ同じ場合はcontinuationTextを出さない（`hasContinuation: false`）。summaryとfullSummaryの前方一致が取れない場合（markdownクリーニングの差異等）は、フォールバックとして十分に長ければfullSummary全体を続きとして表示する（完全なdiffアルゴリズムは実装せず、簡易ヒューリスティックにとどめた）。`app/components/sections/AIOverviewComparisonSection.tsx`のラベルを「詳細を見る」から「続きを見る」に変更。references/referenceSummary/ownDomainReferenced表示・DataForSEOカードの既存表示は変更なし。APIレスポンス形式・summary/fullSummary生成ロジック（`dataforseo_client.py`/`chatgpt_client.py`）・Live API手動確認用ゲート・Render/Vercel環境変数はいずれも変更していない。テストは`app/lib/meta-label.test.ts`に新しい`describe`ブロックを追加（前方一致抽出・改行/空白正規化・`...`/`…`の除去・最小文字数閾値・フォールバック挙動・references等の既存表示への非影響を確認）。詳細は[development_status.md](./development_status.md)参照
- [x] ChatGPTカードで「続きを見る」が出ない時、summary末尾の省略記号を表示上取り除く（`style/summary-ellipsis-without-continuation`、2026-07-28）。ChatGPT (OpenAI API)カードでsummary/fullSummaryの長さが近いケース（例: summary 201文字・fullSummary 214文字）が確認され、`hasContinuation`が`false`（続きが30文字未満のため「続きを見る」を出さない設計、[05_tasks.md](./05_tasks.md)前項参照）になる一方、backend側で短縮された`summary`の末尾に`…`/`...`が残ったままになり「省略されているのに続きを開けない」ように見える問題を修正した。backend/API/OpenAI/DataForSEO providerは一切変更せず、`app/lib/meta-label.ts`の表示用helperのみで対応。`getAiOverviewItemDetailDisplay()`に`displaySummary: string`を追加し、`hasContinuation === true`の場合は`item.summary`をそのまま（末尾の省略記号も残す）、`hasContinuation === false`の場合は新設`stripTrailingEllipsisForDisplay()`（既存の比較用`stripTrailingEllipsis()`に`.trim()`を追加しただけの表示用ラッパー）で末尾の`…`/`...`のみを取り除く（文中の三点リーダーは対象外、正規表現`\s*(\.{3,}|…)\s*$`で末尾のみにマッチ）。`AIOverviewComparisonSection.tsx`の概要表示を`item.summary`から`detail.displaySummary`参照に変更。references/referenceSummary/ownDomainReferenced表示・DataForSEOカードの既存表示・APIレスポンス形式・summary/fullSummary生成ロジックはいずれも変更していない。テストは`app/lib/meta-label.test.ts`に新しい`describe`ブロックを追加（continuation有無での末尾記号の扱い分け、`…`/`...`双方の除去、文中の`…`は非除去、通常文は無変更、201/214文字の実ケース相当、既存references等表示への非影響を確認）。詳細は[development_status.md](./development_status.md)参照
- [x] ChatGPT観測回答をデモ向けに安定化する（`fix/chatgpt-observation-stability`、2026-07-28）。同じブランド名でもChatGPT (OpenAI API)観測の回答・summary/fullSummaryの長さが実行ごとに変わりすぎる課題を受け、依頼者への提出デモに向けて回答の安定性を高めた。新規環境変数`CHATGPT_TEMPERATURE`（`backend/services/chatgpt_settings.py`、デフォルト`0.2`、許可範囲0.0〜1.0、不正値・範囲外は`0.2`にフォールバック）を追加し、OpenAI Responses APIのリクエストボディ（`backend/services/chatgpt_client.py`の`_build_request_body()`）に`temperature`を含めるようにした（`store: false`は維持）。合わせてsystem/userプロンプトを構造化し、systemプロンプトは日本語化した上でWeb検索禁止・簡潔な回答を指示、userプロンプトは「何を提供しているか／主な利用者または用途／代表的な特徴や強み」の3観点を含む3〜5文程度の自然文（箇条書き禁止、参照元・URLを挙げない、不明時は「一般的には十分な情報を確認できません」と述べる）で回答するよう明示的に指示するようにした。**OpenAI API呼び出し回数（1 analyzeあたり最大1回）・ChatGPT providerの2段階安全ゲート（`CHATGPT_PROVIDER_MODE`/`ALLOW_CHATGPT_MODE_OVERRIDE`）・APIキーの扱い（Render環境変数のみ、ログ・レスポンス非露出）・DataForSEO関連コード（`dataforseo_settings.py`/`dataforseo_client.py`/`ai_overview_provider.py`）・Live API手動確認用ゲートはいずれも変更していない**。references取得・Web検索は今回も対象外。テストは`test_chatgpt_settings.py`（temperature未設定/範囲内/範囲外/不正値/境界値0.0・1.0）・`test_chatgpt_client.py`（request bodyへのtemperature反映、systemプロンプトの日本語Web検索禁止文言、userプロンプトの3〜5文指示・参照元なし指示）・`test_chatgpt_provider.py`（設定したtemperatureがリクエストボディに反映されること、未設定時のデフォルト反映）を追加・拡張した。詳細は[backend/README.md](../backend/README.md)「ChatGPT相当モデルの1問観測」、[03_api_design.md](./03_api_design.md)参照
- [x] 短いAI Overview本文は全文表示し、長文のみ「続きを見る」にする（`style/inline-short-ai-overview-summary`、2026-07-28）。ChatGPT (OpenAI API)カードで、概要末尾が「Next....」のように途中で切れ、「続きを見る」を開くと「jsの主要な開発元／メンテナーとしての位置付けでも認識されています。」のように文の続きが挟まって見える問題を修正した。ChatGPT回答はそこまで長くならないことが多いため、短〜中程度の本文は最初から全文表示し、長文の場合だけ折りたたみ表示にする方針にした。backend/API/OpenAI/DataForSEO providerは一切変更せず、`app/lib/meta-label.ts`の表示用helperのみで対応。全文表示しきい値`INLINE_FULL_SUMMARY_MAX_LENGTH`（600文字）を追加し、`getAiOverviewItemDetailDisplay()`を「`item.fullSummary`が存在し600文字以下の場合は`displaySummary = item.fullSummary`・`hasContinuation = false`・`continuationText = undefined`（『続きを見る』自体を表示しない）」「`fullSummary`が600文字超、または`fullSummary`が存在しない場合は既存の`buildContinuationText()`/`stripTrailingEllipsisForDisplay()`ロジックをそのまま使う」という条件分岐に変更した。既存の30文字未満の続きは表示しないルール・`…`/`...`の末尾省略記号処理・ChatGPT/DataForSEO双方への適用はいずれも維持している。`AIOverviewComparisonSection.tsx`は変更不要だった（既に`detail.displaySummary`/`detail.hasContinuation`/`detail.continuationText`を参照する実装だったため）。テストは`app/lib/meta-label.test.ts`に新しい`describe`ブロックを追加（600文字以下での全文表示・`hasContinuation=false`・`continuationText=undefined`の確認、600文字超での既存continuation-toggleロジックへのフォールバック確認、fullSummary非存在時の既存挙動維持、600/601文字の境界値確認、DataForSEOカード相当のreferences/referenceSummary/ownDomainReferenced表示への非影響確認）。既存の`continuation-toggle`関連テストのうち、短いfullSummary例（600文字以下）を使っていたものは、新しい全文表示挙動に合わせて更新するか、`LONG_FILLER`という共有パディング文字列で600文字超に拡張して既存ロジックの検証を維持した。APIレスポンス形式・summary/fullSummary生成ロジックは変更していない。詳細は[development_status.md](./development_status.md)参照
- [x] 明日提出用のデモ状態を固定・確認する（`docs/demo-readiness-checklist`、2026-07-28、docsのみ）。新機能追加ではなく、依頼者への提出デモに向けて既存実装をどう見せるかを整理した。新規[docs/12_demo_readiness.md](./12_demo_readiness.md)を作成し、推奨Render/Vercel Environment Variables（変数名と推奨値のみ、APIキー・パスワードの実値は記載しない）、デモ画面設定の3パターン（DataForSEO+ChatGPT両方／ChatGPTのみ／DataForSEO Sandboxのみ）、デモ用入力例（Vercel/サイボウズ）、デモで見る順番、依頼者への説明文案、明日見せる範囲と次フェーズ扱いの範囲、既知の制約をまとめた。コード変更・OpenAI/DataForSEO呼び出しロジック変更・Live API gate変更はいずれも行っていない
- [x] AI Overview取得モードにliveを明示選択できるようにする（`feature/explicit-dataforseo-live-mode`、2026-07-28）。従来の`dataforseo`モードは`DATAFORSEO_API_ENV`環境変数の値でSandbox/Liveを切り替える設計で、画面上ではどちらが実行されるか分かりにくかった。`AiOverviewProviderMode`（`backend/models.py`、`app/lib/types.ts`）に`dataforseo_sandbox`（`DATAFORSEO_API_ENV`の値に関わらず常にSandboxへ、Liveゲート不要）と`dataforseo_live`（明示的にLiveを要求するが、既存の5つの手動確認用ゲート——`DATAFORSEO_API_ENV=live`・`DATAFORSEO_LIVE_API_ENABLED=true`・`DATAFORSEO_LIVE_CONFIRM_TEXT=ALLOW_DATAFORSEO_LIVE_ONCE`・`DATAFORSEO_REQUEST_LIMIT_PER_ANALYZE=1`・認証情報設定済み——がすべて揃わない限りLiveへは接続しない）を追加した。`backend/services/ai_overview_provider.py`に`_run_dataforseo_sandbox_mode()`/`_run_dataforseo_live_mode()`/`_explicit_live_gate_rejection_reason()`を新設し、ゲート不足時は「DataForSEO Live mode was requested, but ...」で始まる、不足条件が分かる専用の理由文言（`login`/`password`の実値は含まない）を返す。**`backend/services/dataforseo_settings.py`/`dataforseo_client.py`のコア判定ロジック（`is_live_allowed_for_manual_check`等）は変更していない**——新モードはこれらを組み合わせて呼ぶだけ。既存の`dataforseo`モード（env駆動）は後方互換としてそのまま残した。フロント側は`app/lib/types.ts`/`analysis-result-schema.ts`/`app/api/analyze/route.ts`のZodスキーマ・許可リストを拡張し、検証用の「AI Overview取得モード（検証用）」selectorに「DataForSEO Sandbox」「DataForSEO Live」の選択肢を追加した（`app/components/BrandInputForm.tsx`）。dataforseo_live選択時のみ「Liveは課金が発生する可能性があります。Render側のLive許可envが揃っている場合のみ実行されます。」という追加の注意文を表示する。ChatGPT (OpenAI API)観測との併用（`aiOverviewMode`が`"dataforseo_sandbox"`/`"dataforseo_live"`の場合もChatGPTカードが候補になる、`"mock"`のみ引き続きスキップ）も確認済み。テストは`backend/tests/test_ai_overview_provider.py`・`test_main.py`・`app/lib/analysis-request.test.ts`・`app/api/analyze/route.test.ts`に追加（Live gate不足時にHTTPリクエストを送らないことの確認を含む）。詳細は[backend/README.md](../backend/README.md)「AI Overview比較のprovider mode」、[03_api_design.md](./03_api_design.md)参照
- [x] gpt-5系モデルではtemperatureを送らず、ChatGPT観測のHTTP 400を防ぐ（`fix/omit-temperature-for-gpt5-chatgpt-observation`、2026-07-28）。`CHATGPT_MODEL=gpt-5-mini`＋`CHATGPT_TEMPERATURE=0.2`の組み合わせでOpenAI Responses APIがHTTP 400を返し、ChatGPTカードが表示されない不具合が判明した（`gpt-4.1-mini`では同じ`temperature`値で正常動作していたため、gpt-5系モデルへの`temperature`送信が原因と判断）。`backend/services/chatgpt_client.py`に`should_send_temperature(model)`ヘルパーを追加し（モデル名を小文字化・trimした上で`"gpt-5"`から始まるかどうかで判定、大文字小文字を区別しない）、`_build_request_body()`を「`should_send_temperature()`が`True`の場合のみ`temperature`キーをリクエストボディへ追加する」条件分岐に変更した。`model`/`input`/`max_output_tokens`/`store: false`は変更なし。省略時は`temperature was omitted for gpt-5 model compatibility`とログに記録するのみで、`meta.chatgptProvider.reason`やUIには通常表示しない。`backend/services/chatgpt_settings.py`側のバリデーションロジックは変更なし——`CHATGPT_TEMPERATURE`は引き続きモデルに関わらず読み取り・バリデーションされ、送信するかどうかだけを`chatgpt_client.py`が決める。**DataForSEO関連コード・ChatGPT provider mode・request limit・prompt本文はいずれも変更していない**。テストは`test_chatgpt_client.py`（`should_send_temperature()`の判定、`gpt-4.1-mini`/`gpt-4o-mini`でtemperatureが含まれること、`gpt-5-mini`/`gpt-5`/大文字混在`GPT-5-MINI`でtemperatureが含まれないこと、`gpt-5-mini`でも成功レスポンスのパース・APIキー非露出が壊れないこと）・`test_chatgpt_settings.py`（`CHATGPT_MODEL=gpt-5-mini`でも設定値自体は読み取れること）・`test_chatgpt_provider.py`（`CHATGPT_MODEL`未設定＝デフォルトの`gpt-5-mini`でも`build_chatgpt_observation()`経由でtemperatureが送られないことの回帰防止）を追加・更新した。既存テストのうち、`model="gpt-5-mini"`を汎用の動作確認に使っていたものは`gpt-4.1-mini`に置き換えた（temperature関連以外の挙動には影響なし）。詳細は[backend/README.md](../backend/README.md)「ChatGPT相当モデルの1問観測」参照
- [x] Common Crawl最小連携の設計docsを追加（`docs/common-crawl-mvp-design`、2026-07-28、docsのみ）。新規[docs/13_common_crawl_mvp_design.md](./13_common_crawl_mvp_design.md)を作成し、初期MVPはブランド名の全文検索ではなくdomain指定でのCommon Crawl Index検索から開始する方針を整理した（例: `brandName: サイボウズ` / `domain: cybozu.co.jp` で`cybozu.co.jp`配下のURL候補を最大3〜5件探す）。やること/やらないことの範囲、既存Document Pipeline（Provider→Cleaner→Normalizer→Chunker→Analyzer）との接続方法、Provider設計案（`Document.metadata`へのCommon Crawl固有情報格納、`CommonCrawlCandidate`中間型）、Environment Variables案（`COMMON_CRAWL_ENABLED`等、初期値はすべて安全側）、API設計案（`/analyze`統合案Aとdebug用`/common-crawl/search`案Bの比較、Step1のservice+テストから始める推奨順序）、失敗時の扱い（他セクションの解析を止めない、unavailable扱い）、セキュリティ・安全制限（件数・タイムアウト・redirect・HTMLサイズ・URLスキーム制限）、実装ステップ10段階を明文化した。`docs/02_roadmap.md`のPhase 3-2節を更新し、Common Crawl連携のCurrent/Next/Laterを追記した（ルート直下の`roadmap.md`は存在しないため、CLAUDE.mdで「フェーズ別ロードマップ」と明記されている`docs/02_roadmap.md`を更新対象とした）。**コード変更（`app/`/`backend/`）・Environment Variablesの実追加・Render/Vercel設定変更はいずれも行っていない**。詳細は[docs/13_common_crawl_mvp_design.md](./13_common_crawl_mvp_design.md)参照
- [x] Common Crawl settings + Index API clientを実装する（`feature/common-crawl-index-client`、2026-07-28）。[13_common_crawl_mvp_design.md](./13_common_crawl_mvp_design.md)の実装ステップ2〜4（settings追加・Index API client追加・Index検索のみのテスト）にあたる、**Index API検索クライアントのみ**の実装。WARC本文取得・HTML抽出・`Document[]`化・`/analyze`統合・UI追加は今回のスコープ外で、実装していない。新規`backend/services/common_crawl_settings.py`（`load_common_crawl_settings()`。Common Crawlは認証不要の公開データセットのためcredential型・secretは無し）が`COMMON_CRAWL_ENABLED`（デフォルト`false`）・`COMMON_CRAWL_INDEX`（デフォルト`latest`、`CC-MAIN-YYYY-NN`形式も指定可、不正値は`latest`にフォールバック）・`COMMON_CRAWL_MAX_RESULTS`（デフォルト5、1〜10、範囲外は5にフォールバック）・`COMMON_CRAWL_TIMEOUT_SECONDS`（デフォルト10、3〜30、範囲外は10にフォールバック）・`COMMON_CRAWL_USER_AGENT`（デフォルト`AI-Visibility-Platform-MVP`、空文字/200文字超はデフォルトにフォールバック）を読み取る。新規`backend/services/common_crawl_index.py`が`resolve_common_crawl_index(settings)`（`index="latest"`時は`https://index.commoncrawl.org/collinfo.json`を取得し`(year, week)`が最大のindex idを解決、明示indexの場合はcollinfo.jsonへ一切アクセスしない）・`search_common_crawl_domain(domain, settings)`（domainを正規化（scheme/path/port/userinfo除去・小文字化）し危険な値・ドット無しの値を拒否、`https://index.commoncrawl.org/{index}-index`へ`output=json`・`filter=status:200`・`filter=mime:text/html`・`limit`付きでGETし、JSON Lines形式のレスポンスを`CommonCrawlCandidate`（`url`/`timestamp`/`status`/`mime`/`digest`/`length`/`offset`/`filename`/`crawl_index`/`source: "common_crawl"`）へ正規化）を提供する。**このクライアント自体は`COMMON_CRAWL_ENABLED`によるゲート判定を一切持たず、呼ばれれば常に実際にHTTPリクエストを行う**——DataForSEO/ChatGPTクライアントと同じ「クライアントはゲートを持たない、ON/OFF制御はproviderレイヤーの責務」という設計を踏襲しており、`common_crawl_provider.py`相当は今回未実装のため、`COMMON_CRAWL_ENABLED`は現時点では実質的にまだ何のコードパスにも作用しない。結果は`CommonCrawlIndexResult`（`status: "real"|"unavailable"|"off"`、`reason`、`crawl_index`、`candidates`）で表現し、ネットワークエラー・非200・不正JSON・0件はいずれも例外を送出せず`"unavailable"`＋安全な`reason`になる（`"off"`は将来のprovider実装のために型に予約しているのみで、このクライアントからは返らない）。HTML本文・WARC本文は一切保持せず、巨大なレスポンス本文が`reason`に含まれないことをテストで確認済み。テストは`backend/tests/test_common_crawl_settings.py`（27件）・`backend/tests/test_common_crawl_index.py`（25件）を新設し、すべて`httpx.get`をmonkeypatchで差し替え、実際のCommon Crawlへは一切接続していない。`docs/02_roadmap.md`のPhase 3-2節（Current/Doneへ移動）・[13_common_crawl_mvp_design.md](./13_common_crawl_mvp_design.md)（実装状況の追記）・[backend/README.md](../backend/README.md)（新セクション追加）・`.env.example`（`COMMON_CRAWL_*`5変数追加）を合わせて更新した。**DataForSEO/ChatGPT関連コード・`/analyze`のレスポンス形状・frontend（`app/`）・Render/Vercel環境変数はいずれも変更していない**。詳細は[backend/README.md](../backend/README.md)「Common Crawl最小連携」参照
...
- [x] Common Crawlのデータ構造調査（CDXサーバー / WARCファイル / インデックス頻度）（[13_common_crawl_mvp_design.md](./13_common_crawl_mvp_design.md)、settings + Index API client実装、2026-07-28。上記task-log参照。CDX Index API検索までは実装済み、WARCファイル自体の取得は未着手）
- [x] ブランド名に関連するページを絞り込むクエリ・フィルタリング条件の設計（ブランド名の全文検索ではなく、domain指定検索を採用。`common_crawl_index.py`の`search_common_crawl_domain()`、2026-07-28）
