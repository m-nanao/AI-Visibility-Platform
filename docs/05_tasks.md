...
- [x] センチメント分析ロジック（軽量版のみ、2026-07-16）。上記`context_analysis.py`が各文脈カテゴリごとに簡易ポジティブ/ネガティブキーワードの出現数比較で`sentiment`（positive/neutral/negative）を判定する。ただしこれは文脈単位の簡易判定であり、ブランド全体のセンチメント（`BrandSummary.sentimentBreakdown`）は引き続き固定データのまま。より高精度なセンチメント分析（ルールベースの高度化 or 軽量モデル）は今後の検討課題として残る
