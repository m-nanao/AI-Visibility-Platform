...
- 分析ソース内訳表示（`style/analysis-source-breakdown`、2026-07-28）: 「共起語ランキング」カードに「分析ソース: Webページ N件 / Common Crawl補完 N件」という内訳を追加し、入力URLとCommon Crawl補完それぞれの件数がひと目で分かるようにした（Common Crawl offの場合はWebページ件数のみ表示）
