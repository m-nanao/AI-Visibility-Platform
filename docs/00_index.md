...
- [21_analysis_history_ui_design.md](./21_analysis_history_ui_design.md) — **保存済み分析履歴をfrontendで表示するためのUI設計**。`/history`ページ案・一覧の表示項目・既存分析結果コンポーネントの再利用方針・read APIとの接続方針・`READ_HISTORY_ENABLED=false`時の表示方針・`analysisRunId`を`/analyze`レスポンスへ追加しない初期方針を整理する。**履歴一覧UIは実装済みで、本番Vercel環境での動作確認（`READ_HISTORY_ENABLED=false`時の無効表示・`true`時の実データ表示）も完了済み（2026-09-10）。** 履歴詳細UI・`analysisRunId`の`/analyze`レスポンス追加はまだ行っていない。
- [22_analysis_history_detail_ui_design.md](./22_analysis_history_detail_ui_design.md) — **保存済み分析履歴の詳細結果をfrontendで再表示するためのUI設計**。履歴詳細UI実装前に読む。`/history/[id]`ページ案・`GET /analysis-runs/{id}`との接続方針・既存分析結果コンポーネントの再利用方針・保存済み`result_json`をスナップショットとして扱う方針・503/404/schema validation失敗時の表示方針・一覧から詳細への導線を整理する。**設計メモのみで、UI実装・frontend route追加・backend API変更はまだ行っていない。**
