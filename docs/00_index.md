...
- [21_analysis_history_ui_design.md](./21_analysis_history_ui_design.md) — **保存済み分析履歴をfrontendで表示するためのUI設計**。履歴一覧UI実装前に読む。`/history`ページ案・一覧の表示項目・既存分析結果コンポーネントの再利用方針・read APIとの接続方針・`READ_HISTORY_ENABLED=false`時の表示方針・`analysisRunId`を`/analyze`レスポンスへ追加しない初期方針を整理する。**設計メモのみで、初期実装は履歴一覧UIのみに絞り、履歴詳細UIは後続タスク。UI実装・frontend route追加・backend API変更はまだ行っていない。**
