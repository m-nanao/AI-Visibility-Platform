...
- `ownDomainReferenced`（2026-07-23追加）はリクエストの`urls`のドメインと`references`のドメインの単純な文字列一致にとどまり、参照元ページの実際の内容確認・スコアリング・競合ドメインの分類は行わない。`references`自体もDataForSEOが返した`title`/`domain`/`url`等のメタ情報のみで、参照先ページを実際にフェッチして内容を検証することはしない。
