...
- Document Pipeline（Provider→Cleaner→Normalizer→Chunker→Analyzer）の5段階すべてが実装済みになった（Chunkerを2026-07-16に追加）。`Document`型定義・Providerレベルの変換（`user_provided`/`web_fetch`/`development_sample`→`Document[]`）・Cleanerの分離（`backend/services/document_cleaner.py`）・Normalizerの追加（`backend/services/document_normalizer.py`）・Chunkerの追加（`backend/services/document_chunker.py`）・Analyzer側アダプターまで実装済み。同日、軽量な文脈分析（`backend/services/context_analysis.py`）がChunkerの出力を実際に消費する最初のAnalyzerロジックになった。共起解析は引き続き`Document.text`全体を直接読み、Chunker非経由のまま。さらに同日、軽量なブランド認知サマリー（`backend/services/brand_summary.py`）が共起解析・文脈分析の結果から`summary`を実データ由来にし、軽量な改善提案（`backend/services/improvement_suggestions.py`）が共起解析・文脈分析・ブランド認知サマリーの結果から`improvements`を実データ由来にした。2026-07-17には、残る唯一のmockセクションであるAI Overview比較にもprovider切り替え基盤（`backend/services/ai_overview_provider.py`、mock/off/dataforseoモード）が導入され、同日中にDataForSEO認証情報・実行安全ルールの設計（`backend/services/dataforseo_settings.py`）、さらにDataForSEO **Sandbox**への実際のHTTP接続（`backend/services/dataforseo_client.py`）まで実装された。**Live APIへの接続はまだ実装していない**（詳細は下記「実装済み機能」、[11_architecture_v1.md](./11_architecture_v1.md)の「10. 次フェーズ候補」）。
...
- DataForSEO **Sandbox**への実接続（`backend/services/dataforseo_client.py`、2026-07-17、`feature/dataforseo-sandbox-provider`）として`fetch_ai_overview_sandbox()`を実装した。**Live APIへは今回も一切接続しない**——`DATAFORSEO_API_ENV=live`の場合は`DATAFORSEO_LIVE_API_ENABLED`の値に関わらず外部APIを呼ばず`"unavailable"`を返す。エンドポイントは`/v3/serp/google/organic/live/advanced`（Google Organic SERP、Sandbox環境に対してのみ呼び出す）を採用し、レスポンス内の`ai_overview`タイプ項目を探して見つかった場合のみ`"real"`・1件のデータを返す（見つからない・接続失敗の場合は`[]`・`"unavailable"`、いずれも例外を送出せず`/analyze`は常に200を維持）。認証はBasic Auth、実値を保持する新設の`DataForSEOCredentials`型（`dataforseo_settings.py`）をリクエスト直前でのみ使い保存・ログ出力しない。テストはすべて`httpx.post`をmonkeypatchで差し替え、実際のDataForSEO APIへは一切接続していない。`meta.aiOverviewProvider`の形状は変更なしのため、フロントの型・Zodスキーマの変更は不要だった（詳細は[backend/README.md](../backend/README.md)「DataForSEO Sandbox接続」、[07_decisions.md](./07_decisions.md)参照）
